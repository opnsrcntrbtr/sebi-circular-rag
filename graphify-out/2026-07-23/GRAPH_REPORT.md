...
## Corpus Check
- 124 files · ~66,339 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1393 nodes · 2908 edges · 86 communities (71 shown, 15 thin omitted)
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 671 edges (avg confidence: 0.73)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `207fdaae`
...
Cohesion: 0.12
Nodes (26): BaseModel, build_default_pipeline(), _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, load_circulars() (+18 more)
...
Cohesion: 0.11
Nodes (17): main(), demote_superseded(), Lineage, Down-weight reranked (chunk, score) pairs from superseded circulars and     re-s, Map any cited circular that is superseded -> the circular(s) superseding it., Connected component over supersedes/superseded_by (both tiers)., The circular in this family that governs on date as_of (ISO), or         None wh, superseded_citations() (+9 more)
...
### Community 21 - "Chunk"
Cohesion: 0.11
Nodes (23): ExternalSpaceGenerator, HFGenerator, HybridGenerator, CPU / remote generation for the Hugging Face Spaces demo.  All classes implement, External Space first; on ANY failure fall back to the local CPU model.      exte, Primary generator: calls a public LLM Space via gradio_client.      Wired to hug, Fallback generator: small instruct model via transformers on CPU., Central configuration: config.toml defaults, overridden by SEBI_RAG_* env vars. (+15 more)
...
Cohesion: 0.19
Nodes (29): main(), smoke_pipeline(), run_retrieval_benchmark(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., build_lineage() (+21 more)
...
Cohesion: 0.13
Nodes (16): Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the per, Embedder protocol + a deterministic test embedder + the real bge-m3 embedder.  T, Answer, faithfulness(), Generation with a hard abstention gate (D5).  If the top reranked score is below, Check that every circular id the answer cites (in square brackets) was     actua (+8 more)
...
Cohesion: 0.06
Nodes (28): Embedder, ndarray, _tokens(), HydeExpander, HyDE (Hypothetical Document Embeddings): query -> statutory passage.  Part B of, DenseIndex, _doc_checksum(), ndarray (+20 more)
...
### Community 48 - ".encode"
Cohesion: 0.23
Nodes (9): get_pipeline(), Cache one pipeline per mode; both share retriever/reranker/lineage., build_spaces_pipeline(), _cpu_env(), Pipeline builder for the Hugging Face Spaces demo (CPU-only, Linux).  Parallel t, HybridRetriever, Path, _chunks() (+1 more)
...
Cohesion: 0.23
Nodes (9): _doc(), EvalReport, Golden-set evaluation harness (P1).  Runs the pipeline over a labelled golden se, report_dict(), run_eval(), SEBI Circular RAG — local-first, Apple Silicon.  Pipeline: ingest -> segment ->, P1 evaluation-harness test (offline).  Loads the real seed corpus (data/corpus/c, test_eval_harness_metric_suite() (+1 more)
...
Cohesion: 0.22
Nodes (6): Emit one JSON line of retrieval/citation/abstention metrics over golden_v5 (env, Stage-1 hybrid retrieval: dense (FAISS) + sparse (BM25) fused by RRF.  Mandatory, Reciprocal Rank Fusion. Rank-only — sidesteps score-scale mismatch., rrf_fuse(), Index persistence round-trip (offline)., test_rrf_fusion_orders_by_reciprocal_rank()
...
- **Why does `Chunk` connect `test_gate.py` to `Core RAG Pipeline`, `Benchmark Infrastructure`, `.grounded`, `Lineage`, `Master Metadata`, `lineage.py`, `Chunk`, `Answer Generation`, `test_gate.py`, `annotate_corpus`, `bench_rerankers.py`, `.encode`, `_compute_kwargs`, `paired_delta`, `build_index.py`, `test_integration_e2e.py`, `faithfulness`, `paired_delta`, `detect_relations_ex`, `test_gate.py`?**
  _High betweenness centrality (0.126) - this node is a cross-community bridge._
- **Why does `main()` connect `Benchmark Infrastructure` to `Core RAG Pipeline`, `Data Processing`?**
