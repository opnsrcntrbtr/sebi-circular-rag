...
## Summary
- 1088 nodes · 1944 edges · 77 communities (61 shown, 16 thin omitted)
- Extraction: 79% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 297 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `45dee1eb`
...
8. `SpacesSettings` - 17 edges
9. `ExtractiveStubGenerator` - 17 edges
10. `HybridRetriever` - 17 edges
...
- `_Canned` --uses--> `Settings`  [INFERRED]
  tests/test_spaces.py → src/sebi_rag/settings.py
- `settings()` --calls--> `Settings`  [INFERRED]
  tests/test_spaces.py → src/sebi_rag/settings.py
...
Cohesion: 0.08
Nodes (27): build_ui(), get_pipeline(), Hugging Face Spaces entrypoint — SEBI Circular RAG demo (CPU-only).  Gradio SDK, Cache one pipeline per mode; both share retriever/reranker/lineage., run_query_spaces(), ExternalSpaceGenerator, HFGenerator, HybridGenerator (+19 more)
...
Cohesion: 0.26
Nodes (13): smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, LexicalReranker, Deterministic query-coverage reranker (test/fallback).      Score = fraction of, CircularMeta, hierarchical_chunk(), Document -> section -> paragraph chunks with stable IDs.      A "section" is det (+5 more)

### Community 4 - "Safety Gates & Abstention"
Cohesion: 0.14
Nodes (11): Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Build the dense+sparse index once and persist it (run after corpus changes)., Calibrate top_k and the abstention threshold against the citation-precision sign, contexts_for(), ADR-002 follow-up: compare the production subject-sim gate against the SECTION-A, Emit one JSON line of retrieval/citation/abstention metrics over golden_v5 (env, Embedder protocol + a deterministic test embedder + the real bge-m3 embedder.  T (+3 more)
...
Cohesion: 0.20
Nodes (16): BaseModel, build_default_pipeline(), _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, BGEM3Embedder (+8 more)
...
Cohesion: 0.05
Nodes (38): (a) Quality bump — larger MLX model  — DONE (2026-07-01), (b) Packaging / deployment  — DONE (2026-07-01), (c) Grow the corpus via the scraper  — IMPLEMENTED (2026-07-01), Next Steps — Structured Plans, Completed, Current Snapshot, Current Validation Step, Known Blockers (+30 more)
...
Cohesion: 0.09
Nodes (23): 10. Extending, 11. Troubleshooting, 12. Testing, 13. Legal & safety notes, 1. Concepts at a glance, 2. Requirements & setup, 3. Directory structure, 4.1 Scrape circulars (runs on your machine) (+15 more)
...
Cohesion: 0.07
Nodes (45): answer_with_abstention(), ExtractiveStubGenerator, faithfulness(), _grounded_prompt(), Judge, _judge_prompt(), _judge_prompt_identify(), MLXJudge (+37 more)
...
Cohesion: 0.21
Nodes (8): FastAPI, create_app(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit, metadata, test_auth_required_when_key_set(), test_citation_meta_reports_superseded(), test_query_exceeds_time_budget_returns_504(), test_rate_limit(), _tiny_pipeline()
...
Cohesion: 0.24
Nodes (10): Path, main(), Create/update the Gradio-SDK Hugging Face Space for the CPU demo and push app.py, Settings.load() plus the [spaces] table as settings.spaces.*          Load order, _clear(), Settings: defaults, config.toml, and env-override precedence., test_defaults_when_no_file(), test_env_overrides() (+2 more)

### Community 63 - "faithfulness"
Cohesion: 0.11
Nodes (24): main(), main(), Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped to, annotate_corpus(), build_lineage(), _currency(), demote_superseded(), detect_relations() (+16 more)

### Community 69 - "app.py"
Cohesion: 0.29
Nodes (10): Protocol, Answer, Generator, Lineage, Map any cited circular that is superseded -> the circular(s) superseding it., superseded_citations(), RAGPipeline, End-to-end wiring: segment -> hybrid retrieve -> rerank -> generate/abstain. (+2 more)

### Community 70 - "SEBI Circular RAG — Hugging Face Spaces demo"
Cohesion: 0.17
Nodes (8): _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, _ollama_up(), Step 12 — end-to-end RAG integration test with the REAL stack.  bge-m3 (MPS) + b, _chunks(), Index persistence round-trip (offline)., test_index_save_load_roundtrip()
...
## Knowledge Gaps
- **226 isolated node(s):** `How this demo differs from the full local system`, `UI modes`, `ZeroGPU-hardware workaround`, `Deploying`, `Data, licensing and citation` (+221 more)
...
- **Why does `Chunk` connect `answer_with_abstention` to `Evaluation & Benchmarking`, `Embeddings & Grounding`, `API & Data Ingestion`, `Safety Gates & Abstention`, `app.py`, `Reranker Evaluation`, `SEBI Circular RAG — Hugging Face Spaces demo`, `5. Running the service`, `Qwen3 Reranking`, `pipeline.py`, `generate.py`, `Injection Testing`, `faithfulness`?**
  _High betweenness centrality (0.110) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `app.py` to `Embeddings & Grounding`, `API & Data Ingestion`, `Reranker Evaluation`, `Qwen3 Reranking`, `generate.py`, `answer_with_abstention`, `today-2026-07-10.md`, `faithfulness`?**
