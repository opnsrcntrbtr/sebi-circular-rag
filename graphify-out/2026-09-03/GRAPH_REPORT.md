...
## Summary
- 3313 nodes · 6867 edges · 199 communities (159 shown, 40 thin omitted)
- Extraction: 76% EXTRACTED · 24% INFERRED · 0% AMBIGUOUS · INFERRED: 1620 edges (avg confidence: 0.75)
...
3. `ExtractiveStubGenerator` - 59 edges
4. `hierarchical_chunk()` - 55 edges
5. `HashEmbedder` - 47 edges
...
Cohesion: 0.10
Nodes (39): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design, main(), main(), main(), ADR-004 adoption: calibrate abstain_threshold for jina-reranker-v3-mlx's score s, main() (+31 more)
...
Cohesion: 0.06
Nodes (60): _aggregate(), eligible(), main(), _measure(), phase_generate(), phase_report(), B' citation-scorer cohort measurement: control (bge, pointwise) vs J1 (jina, lis, Answerable, non-as_of, with gold citations. Matches warrant_scorer_cohort.py's (+52 more)
...
Cohesion: 0.11
Nodes (44): run(), Build a lightweight pipeline for --smoke mode.      Uses a stub retriever (no FA, smoke_pipeline(), smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required. (+36 more)
...
Cohesion: 0.08
Nodes (22): FastAPI, _citation_meta(), create_app(), Truncate chunk text for a response payload; append an ellipsis if cut., Build one CitationMeta per unique circular (first-seen chunk wins).      chunk_i, _truncate_preview(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit, metadata, /ready should trigger pipeline build and return ready=true. (+14 more)
...
Cohesion: 0.12
Nodes (35): is_degenerate(), Re-score `pool` with a rewritten query when `reranked` is below `floor`.      Re, Fixed rewrite, for tests and for replaying a preregistered rewrite., True when `rewritten` is unusable and the rescue should be abandoned.      Degen, rescue_pool(), StaticQueryRewriter, _chunk(), _EchoGenerator (+27 more)
...
Cohesion: 0.08
Nodes (35): _record(), _jaccard(), load_regulations(), name_tokens(), Path, Regulation identity + name resolution (spec 2026-07-23 §3.2, §3.6).  Regulations, Resolve a cited regulation name+year to a canonical reg_id.      Returns (reg_id, Load data/corpus/regulations.jsonl into a list of regulation records.      Thin (+27 more)
...
Cohesion: 0.20
Nodes (15): _chunk(), test_expand_sparse_off_routes_raw_query_to_sparse_leg(), test_retrieve_dense_leg_keeps_raw_query(), test_retrieve_routes_expanded_query_to_sparse_leg(), _chunks(), Index persistence round-trip (offline)., F-01/F-02 fix: save() must record which embedder produced the index., F-03 fix: save() must record which segment.py version chunked the     index, so (+7 more)
...
Cohesion: 0.09
Nodes (39): contexts_for(), demote_superseded(), detect_relations(), detect_relations_ex(), Down-weight reranked (chunk, score) pairs from superseded circulars and     re-s, Like detect_relations, but returns dict records with evidence spans., Return (relation, referenced_circular) for each distinct reference., _window() (+31 more)
...
### Community 40 - "Chunk"
Cohesion: 0.13
Nodes (11): Chunk, _grounded_prompt(), Max cosine(query, doc subject line) over contexts — the primary         gate sig, Max cosine(query, section heading) over contexts — the second tier., F4 (ADR-001): retrieved text is explicitly delimited as quoted DATA and     the, subject_sim == threshold (0.42) passes the gate (>= comparison)., section_score == section_threshold (0.60) passes via second tier., test_section_score_exactly_at_threshold_passes() (+3 more)
...
Cohesion: 0.13
Nodes (22): main(), parse_args(), Namespace, Compare query-expansion arms (current prod / no-expand / HyDE) on a golden set., run_arm(), fmt(), mean_or_none(), ndcg_at_k() (+14 more)
...
Cohesion: 0.18
Nodes (24): beir_corpus_rows(), beir_query_rows(), build_golden_v6(), dir_fingerprint(), enrich_golden_item(), export_beir(), git_commit(), Any (+16 more)
...
Cohesion: 0.10
Nodes (20): Embedder, ndarray, _tokens(), DenseIndex, _doc_checksum(), _embedder_identity(), ndarray, Path (+12 more)

### Community 57 - "Lineage"
Cohesion: 0.13
Nodes (14): _extracts(), MLXQueryRewriter, query_rewriter_for(), QueryRewriter, Paraphrase rescue for the cross-encoder score floor.  Preregistered in `docs/sup, Factory mirroring `generate.citation_scorer_for`: None when disabled., Rewrites a lay-vocabulary query into statutory vocabulary.      Returns None whe, Local MLX-LM rewriter. Greedy decoding -> deterministic. (+6 more)
...
Cohesion: 0.09
Nodes (30): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production?  The 202, Preregistered cohort measurement for the CE paraphrase rescue.  Spec: `docs/supe, What does the 0.05 cross-encoder score floor actually catch?  `Settings.abstain_, cited_docs(), metrics(), Capture-once margin sweep for B' selective citations.  One pipeline pass over th, log(), Margin sweep for B' selective citations on the golden_v7 adjudicated set.  One m (+22 more)
...
### Community 62 - "api.py"
Cohesion: 0.11
Nodes (27): BaseModel, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the roadm, main(), What actually makes a context window large: chunk size, or chunk count?  Read-on, build_default_pipeline(), CitationMeta, QueryRequest (+19 more)
...
Cohesion: 0.09
Nodes (21): Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., _is_table_row_candidate(), _is_table_row_filler(), _is_toc_row_candidate(), _merge_table_rows(), _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, Split into units each <= max_chars.      PDF-extracted text often lacks blank-li (+13 more)
...
Cohesion: 0.18
Nodes (10): Protocol, Answer, Generator, Judge, Map any cited circular that is superseded -> the circular(s) superseding it., superseded_citations(), _LineageAwareReranker, Reranker wrapper that re-applies lineage handling to its output.      The paraph (+2 more)
...
Cohesion: 0.14
Nodes (24): get_pipeline(), Cache one pipeline per mode; both share retriever/reranker/lineage., _compute_kwargs(), _embed_kwargs(), _compute_kwargs() plus model_path - BGEM3Embedder-only, never spread     into Cr, Resolve device/fp16/batch for the torch embedder + reranker.      Shared verbati, build_spaces_pipeline(), _cpu_env() (+16 more)
...
Cohesion: 0.33
Nodes (11): answer_with_abstention(), _chunk(), Offline tests for the ADR-002 certainty architecture: abstention reasons, confid, test_advisory_draft_on_gate_failure_only_when_requested(), test_certainty_capped_medium_without_gate(), test_certainty_high_when_subject_sim_strong_and_faithful(), test_no_context_reason_when_top_k_zero(), test_score_floor_reason() (+3 more)
...
### Community 136 - "main"
Cohesion: 0.47
Nodes (5): main(), parse_args(), Namespace, Reranker interaction check: does the turns-1-3 winning fusion/pool config change, run_arm()
...
### Community 184 - "TestPerQueryRecall"
Cohesion: 0.50
...
## Knowledge Gaps
- **58 isolated node(s):** `checks.sh script`, `measure.sh script`, `autoresearch.sh script`, `PYTHONPATH`, `TOKENIZERS_PARALLELISM` (+53 more)
...
- **Why does `Chunk` connect `Lineage` to `load_golden`, `test_selective_citations.py`, `RAGPipeline`, `Frame`, `SpladeIndex`, `test_paraphrase_rescue.py`, `test_attribution.py`, `test_spaces.py`, `SubjectSimJudge`, `context_headers.py`, `demote_superseded`, `test_spaces_app.py`, `test_rerank_set_encoder.py`, `test_hyde.py`, `benchmark.py`, `hierarchical_chunk`, `HybridRetriever`, `test_rerank_jina_v3.py`, `generate.py`, `test_context_recall.py`, `api.py`, `test_benchmark.py`, `build_default_pipeline`, `ValueError`, `test_expand.py`, `validate_golden_v7`, `Qwen3MLXReranker`, `scrape_regulations.py`, `build_spaces_pipeline`, `context_headers.py`, `test_injection.py`, `resolve_chunk_spans`, `main`?**
  _High betweenness centrality (0.105) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `load_golden`, `eval_json.py`, `measure_mrr`, `measure_parsing_latency`, `measure_temporal_accuracy`, `Frame`, `test_api.py`, `test_paraphrase_rescue.py`, `build_report`, `measure_retrieval_recall`, `benchmark.py`, `HybridRetriever`, `Lineage`, `generate.py`, `api.py`, `MeasureResult`, `ValueError`, `test_eval_harness_v7.py`, `Path`, `build_spaces_pipeline`, `hybrid_gate_sweep.py`, `measure_supersession_precision`, `resolve_chunk_spans`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `load_golden` to `seed_v7.py`, `test_golden_v7_packet.py`, `test_selective_citations.py`, `run_judge`, `Frame`, `Settings`, `test_finetune_eval_phase0.py`, `test_conformal.py`, `test_corpus.py`, `gemini_adjudicate.py`, `benchmark.py`, `test_finetune_holdout.py`, `agreement.py`, `generate.py`, `test_measure.py`, `api.py`, `adjudicate`, `eval_harness.py`, `adjudicate_draft.py`, `hybrid_gate_sweep.py`, `phase_judge`, `main`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Are the 57 inferred relationships involving `Chunk` (e.g. with `NLIAttributionScorer` and `BenchmarkIssue`) actually correct?**
...
- **Are the 53 inferred relationships involving `ExtractiveStubGenerator` (e.g. with `get_pipeline()` and `run()`) actually correct?**
  _`ExtractiveStubGenerator` has 53 INFERRED edges - model-reasoned connections that need verification._
- **Are the 43 inferred relationships involving `hierarchical_chunk()` (e.g. with `smoke_pipeline()` and `_distinct_pipeline()`) actually correct?**
  _`hierarchical_chunk()` has 43 INFERRED edges - model-reasoned connections that need verification._