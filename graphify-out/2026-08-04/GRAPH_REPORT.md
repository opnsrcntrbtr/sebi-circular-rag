...
## Corpus Check
- 164 files · ~167,815 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2063 nodes · 4250 edges · 144 communities (117 shown, 27 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 202 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `63c98009`
...
3. `HashEmbedder` - 42 edges
4. `RAGPipeline` - 39 edges
5. `CircularMeta` - 39 edges
6. `ExtractiveStubGenerator` - 31 edges
...
- `_CannedGenerator` --uses--> `Settings`  [INFERRED]
  tests/test_api.py → src/sebi_rag/settings.py
- `_SlowGenerator` --uses--> `Settings`  [INFERRED]
  tests/test_api.py → src/sebi_rag/settings.py
...
Cohesion: 0.14
Nodes (16): Benchmark MLX generators on the golden set: faithfulness, groundedness,…, Retrieval-only benchmark with TREC runfile and reproducibility metadata. Use…, scripts/eval_asof.py, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the…, Emit one JSON line of retrieval/citation/abstention metrics using the persisted, Embedder protocol + a deterministic test embedder + the real bge-m3 embedder.…, faithfulness(), Generation with a hard abstention gate (D5).  If the top reranked score is below (+8 more)
...
Cohesion: 0.12
Nodes (24): Generator, ExternalSpaceGenerator, HFGenerator, HybridGenerator, CPU / remote generation for the Hugging Face Spaces demo. All classes implement…, External Space first; on ANY failure fall back to the local CPU model.…, Primary generator: calls a public LLM Space via gradio_client. Wired to…, Fallback generator: small instruct model via transformers on CPU. (+16 more)

### Community 6 - "Chunking and Embedding"
Cohesion: 0.14
Nodes (18): _build_chunks(), _build_pipeline(), _FixedReranker, Minimal end-to-end test of the SEBI RAG pipeline. Runs fully offline…, Offline pipeline whose single circular rests on a repealed regulation., Deterministic reranker: score by doc_id lookup (test-only)., citations = all top_k contexts, so a demoted superseded chunk deep in the…, Giant-family regression (golden asof-p8): OLD is superseded by two same-day… (+10 more)
...
Cohesion: 0.13
Nodes (32): main(), main(), Create the enriched golden_v6 benchmark seed from frozen golden_v5. This does…, beir_corpus_rows(), beir_query_rows(), BenchmarkIssue, build_golden_v6(), chunks_by_doc() (+24 more)

### Community 12 - "Hybrid Retrieval Indexing"
Cohesion: 0.14
Nodes (12): Embedder, Protocol, DenseIndex, _doc_checksum(), HybridRetriever, ndarray, Path, F3 (ADR-001): encode only new/changed documents; reuse cached embedding rows… (+4 more)
...
Cohesion: 0.20
Nodes (12): _lin_chain(), P2 lineage / supersession resolution tests., test_build_lineage_edges_tiered(), test_build_lineage_inferred_master_topic_edge(), test_governing_on_before_family_exists(), test_governing_on_linear_chain(), test_governing_on_unknown_dates_excluded(), test_lineage_load_old_file_defaults_empty_edges() (+4 more)
...
Cohesion: 0.31
Nodes (12): BaseModel, CitationMeta, _compute_kwargs(), QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, Resolve device/fp16/batch for the torch embedder + reranker., RegulationRef (+4 more)
...
Cohesion: 0.09
Nodes (17): FastAPI, integration, Lineage, RAGPipeline, _citation_meta(), create_app(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit,…, /ready should trigger pipeline build and return ready=true. (+9 more)
...
Cohesion: 0.29
Nodes (10): evaluate(), run_retrieval_benchmark(), _aggregate(), _doc(), _eval_item(), EvalReport, _mean(), Golden-set evaluation harness (P1). Runs the pipeline over a labelled golden… (+2 more)

### Community 28 - "Faithfulness and Abstention"
Cohesion: 0.19
Nodes (17): Path, build_default_pipeline(), _as_bool(), _get(), Central configuration: config.toml defaults, overridden by SEBI_RAG_* env vars., Settings.load() plus the [spaces] table as settings.spaces.*          Load order, Resolve a setting: env var > config dict > default., Coerce a config/env value to bool. Env vars arrive as strings; toml/default (+9 more)
...
Cohesion: 0.18
Nodes (15): _chunk(), _FakeReranker, Tests for B' selective citations: select_citations() and its integration., Deterministic scorer: returns preset answer-relevance scores, sorted desc., When citation_scorer_enabled=True, Settings loads a non-None scorer., When citation_scorer_enabled=False, Settings loads scorer disabled., _reranked(), test_always_keeps_at_least_one_when_all_below_margin() (+7 more)
...
Cohesion: 0.06
Nodes (33): derive_floors(), main(), Derive CI gate floors from the golden_v7 adjudicated subset (spec sec 8).  Write, metric -> per-query score vector, into gate-floor names -> floor value.      Met, floors_ok(), Path, Which golden set gates CI, and whether its adjudicated subset clears the…, Resolution order: explicit SEBI_RAG_GOLDEN override, then the armed v7 gate,… (+25 more)
...
Cohesion: 0.12
Nodes (15): auroc(), best_threshold(), evaluate(), main(), F2 (ADR-001): benchmark rerankers on golden_v5 with cluster-separation metrics.…, P(pos_score > neg_score); ties count half. pos = answerable top-scores, neg =…, Threshold maximising abstention accuracy: answer if score >= thr. Returns (thr,…, qwen3_rerank_prompt() (+7 more)
...
Cohesion: 0.05
Nodes (46): Chunk, Protocol, Answer, answer_with_abstention(), _is_non_sebi_domain(), Judge, _judge_prompt(), _judge_prompt_identify() (+38 more)
...
Cohesion: 0.22
Nodes (9): load_circulars(), Path, measure_parsing_latency(), Measure PDF ingestion throughput (chars/sec, ms/PDF). Samples 20 PDFs…, CircularMeta, _HallucinatingGenerator, test_pipeline_flags_hallucinated_citation(), test_numeric_miner_requires_numeric_pattern() (+1 more)
...
Cohesion: 0.24
Nodes (10): dataset_quality(), load_index_chunks(), main(), Path, Export benchmark artifacts for retrieval/RAG/data-quality evaluation. Outputs:…, write_card(), main(), Candidate pools for chunk-label judging (spec §6). TREC-style pooling: union of… (+2 more)
...
Cohesion: 0.25
Nodes (5): Build the dense+sparse index once and persist it (run after corpus changes).…, Calibrate top_k and the abstention threshold against the citation-precision…, Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., sebi_rag/__init__.py, SEBI Circular RAG — local-first, Apple Silicon. Pipeline: ingest -> segment ->…
...
Cohesion: 0.16
Nodes (29): Embedder, Reranker, Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, Build the full pipeline with real models., real_pipeline(), smoke_pipeline(), smoke_pipeline(), HashEmbedder (+21 more)

### Community 125 - "_s_mc_no"
Cohesion: 0.15
Nodes (10): Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped…, contexts_for(), ADR-002 follow-up: compare the production subject-sim gate against the SECTION-…, _currency(), demote_superseded(), mc_topic(), P2 — cross-document supersession resolution. Classifies each circular's…, Normalised topic of a 'Master Circular for/on <TOPIC>' title, else None. Used… (+2 more)
...
Cohesion: 0.16
Nodes (7): _grounded_prompt(), F4 (ADR-001): retrieved text is explicitly delimited as quoted DATA and     the, CrossEncoderReranker, Protocol, Production reranker: bge-reranker-v2-m3 via sentence-transformers CrossEncoder…, Reranker, Chunk
...
- **Why does `Chunk` connect `CrossEncoderReranker` to `Benchmarking and Indexing`, `Web UI and Demo`, `Benchmark Dataset Generation`, `Hybrid Retrieval Indexing`, `test_persistence.py`, `Contextual Header Generation`, `Hugging Face Pipeline`, `Configuration and Settings`, `Golden Dataset Validation`, `HyDE Query Expansion`, `Query Lexical Expansion`, `Qwen MLX Reranker`, `Document ID Remapping`, `Golden Dataset Seeding`, `Prompt Injection Detection`, `answer_with_abstention`, `Span to Chunk Resolution`, `Benchmark Artifact Export`, `Benchmark Export Testing`, `_grounded_prompt`, `_rejoin_split`, `_s_mc_no`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `_rejoin_split` to `Evaluation Harness Testing`, `Benchmarking and Indexing`, `MRR Evaluation`, `Chunking and Embedding`, `Evaluation Metrics Reporting`, `Retrieval Recall Evaluation`, `Temporal Accuracy Evaluation`, `Retrieval Metric Evaluation`, `Benchmark Dataset Generation`, `.grounded`, `TestPerQueryRecall`, `.query`, `Supersession Precision Measurement`, `Temporal Evaluation Runner`, `answer_with_abstention`, `Hugging Face Pipeline`, `Reranker Evaluation Harness`, `TestReadTrecRun`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `sebi_rag/eval_asof.py` connect `Temporal Evaluation Runner` to `build_report`, `Benchmarking and Indexing`, `test_gate.py`, `_rejoin_split`, `Benchmark Uncertainty Quantification`, `Binomial Confidence Intervals`, `_s_mc_no`?**
...
- **Are the 9 inferred relationships involving `CircularMeta` (e.g. with `MeasureReport` and `MeasureResult`) actually correct?**
  _`CircularMeta` has 9 INFERRED edges - model-reasoned connections that need verification._