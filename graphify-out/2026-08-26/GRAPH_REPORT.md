...
## Corpus Check
- 232 files · ~213,119 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2848 nodes · 6011 edges · 175 communities (138 shown, 37 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 622 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b39d3771`
...
Cohesion: 0.09
Nodes (29): build_aikosh_pack(), build_chunk_rows(), build_citation_pairs(), build_corpus_rows(), build_eval_rows(), build_hf_card(), build_kaggle_metadata(), build_lineage_rows() (+21 more)

### Community 2 - "test_trecio.py"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (31): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production? The…, Preregistered cohort measurement for the CE paraphrase rescue. Spec:…, What does the 0.05 cross-encoder score floor actually catch?…, Benchmark MLX generators on the golden set: faithfulness, groundedness,…, Retrieval-only benchmark with TREC runfile and reproducibility metadata. Use…, Build the dense+sparse index once and persist it (run after corpus changes).…, CE_MISMATCH false-abstention investigation (score-floor-diagnostic 2026-08-18).…, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the… (+23 more)
...
Cohesion: 0.08
Nodes (52): citation_scorer_for(), The single enable/disable AND backend decision for B'. Returns None when…, Callable compatible with select_citations' scorer.rerank() signature. Wraps…, Context ids the answer rests on. Scores each context via `scorer`, keeps those…, select_citations(), warrant_scorer(), _chunk(), _FakeReranker (+44 more)
...
### Community 13 - "hierarchical_chunk"
Cohesion: 0.16
Nodes (18): hierarchical_chunk(), Document -> section -> paragraph chunks with stable IDs. A "section" is…, test_numeric_miner_requires_numeric_pattern(), test_paraphrase_skips_preamble_and_short_chunks(), pipeline(), fixture, _body(), Chunker (segment.hierarchical_chunk) behaviour. Regression guard for the "5.… (+10 more)
...
Cohesion: 0.09
Nodes (44): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design…, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), main(), main() (+36 more)

### Community 22 - "Settings"
Cohesion: 0.12
Nodes (30): is_master(), main(), Is the eval set measuring retrieval, or measuring its own construction? Read-…, main(), P0 prep: price a larger MLX generator before committing to the R0 upgrade.…, rss_gb(), main(), main() (+22 more)
...
### Community 24 - "sweep_citation_margin.py"
Cohesion: 0.24
Nodes (10): log(), Margin sweep for B' selective citations on the golden_v7 adjudicated set. One…, run(), Derive CI gate floors from the golden_v7 adjudicated subset (spec sec 8).…, One scoring path shared by `eval_json.py` (which measures) and…, Score one golden row through the production-shaped pipeline. Returns per-row…, Per-row records -> metric -> score vector, skipping rows where the metric was…, score_row() (+2 more)
...
### Community 30 - "api.py"
Cohesion: 0.07
Nodes (31): BaseModel, FastAPI, integration, _citation_meta(), CitationMeta, _compute_kwargs(), create_app(), QueryRequest (+23 more)

### Community 31 - ".build"
Cohesion: 0.07
Nodes (26): Embedder, ndarray, Protocol, _tokens(), DenseIndex, _doc_checksum(), ndarray, F3 (ADR-001): encode only new/changed documents; reuse cached embedding rows… (+18 more)
...
Cohesion: 0.09
Nodes (55): Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), smoke_pipeline(), run_retrieval_benchmark(), load_circulars(), Path, HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable… (+47 more)
...
Cohesion: 0.12
Nodes (31): contexts_for(), demote_superseded(), detect_relations(), Down-weight reranked (chunk, score) pairs from superseded circulars and re-…, Return (relation, referenced_circular) for each distinct reference., _lin_chain(), P2 lineage / supersession resolution tests., OLD_E superseded by an explicit clause; OLD_I only by a title heuristic. (+23 more)

### Community 62 - "test_benchmark.py"
Cohesion: 0.43
...
Cohesion: 0.14
Nodes (13): detect_relations_ex(), Like detect_relations, but returns dict records with evidence spans., _window(), measure_supersession_precision(), Measure fraction of detected supersession edges that are genuine. Samples…, Verify a supersession edge by cross-referencing corpus records. Returns "true",…, _verify_supersession_edge(), A circular that names another circular BEFORE the supersede trigger word must… (+5 more)

### Community 71 - "test_eval_generator.py"
Cohesion: 0.18
...
### Community 79 - "test_golden_v7_resolver.py"
Cohesion: 0.42
...
Cohesion: 0.12
Nodes (28): main(), parse_args(), Namespace, Compare query-expansion arms (current prod / no-expand / HyDE) on a golden set.…, run_arm(), fmt(), mean_or_none(), ndcg_at_k() (+20 more)

### Community 85 - "remap_doc_ids.py"
Cohesion: 0.33
...
Cohesion: 0.09
Nodes (28): doc_ids_deduped(), [(chunk_idx, score), ...] -> distinct doc_ids in rank order., aggregate(), eligible(), main(), measure(), Preregistered cohort measurement for supersession confidence tiering. Spec:…, Answerable, non-as_of, with gold citations: the rows citation metrics exist for. (+20 more)
...
### Community 101 - "test_context_recall.py"
Cohesion: 0.46
...
### Community 116 - "measure_context_precision"
Cohesion: 0.50
...
### Community 126 - "Chunk"
Cohesion: 0.10
Nodes (13): Generator, Judge, MLXGenerator, OllamaGenerator, Protocol, Apple-Silicon-native generation via MLX-LM (D6 preferred runtime). Loads a…, Grounded generation via local Ollama (D6 canonical runtime option).…, _extracts() (+5 more)

### Community 127 - "run_all_metrics"
Cohesion: 0.29
...
### Community 131 - "superseded_citations"
Cohesion: 0.67
...
### Community 161 - "test_golden_v7_agreement.py"
Cohesion: 0.19
...
- **Why does `Chunk` connect `Chunk` to `test_paraphrase_rescue.py`, `generate.py`, `SpladeIndex`, `test_selective_citations.py`, `hierarchical_chunk`, `test_attribution.py`, `context_headers.py`, `HybridRetriever`, `.query`, `.build`, `answer_with_abstention`, `benchmark.py`, `test_rerank_jina_v3.py`, `test_expand.py`, `test_spaces.py`, `RAGPipeline`, `validate_golden_v7`, `SubjectSimJudge`, `test_lineage.py`, `test_benchmark.py`, `SpacesSettings`, `corpus_spaces.py`, `.grounded`, `Qwen3MLXReranker`, `WarrantJudge`, `test_injection.py`, `test_context_recall.py`, `main`, `JinaMLXReranker`?**
  _High betweenness centrality (0.093) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `test_paraphrase_rescue.py`, `generate.py`, `hierarchical_chunk`, `HybridRetriever`, `sweep_citation_margin.py`, `sebi_rag/eval_asof.py`, `.query`, `.build`, `api.py`, `benchmark.py`, `measure_supersession_precision`, `Lineage`, `test_eval_harness_v7.py`, `eval_harness.py`, `measure_parsing_latency`, `measure.py`, `measure_context_precision`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `Chunk`, `run_all_metrics`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `HybridRetriever` to `run_judge`, `eval_harness.py`, `gemini_adjudicate.py`, `adjudicate_draft.py`, `test_golden_v7_packet.py`, `Frame`, `benchmark.py`, `agreement.py`, `MeasureResult`, `phase_generate`, `local_adjudicate.py`, `backfill_escalations.py`, `main`, `seed_v7.py`, `test_conformal.py`, `remap_doc_ids.py`, `Settings`, `RAGPipeline`?**
