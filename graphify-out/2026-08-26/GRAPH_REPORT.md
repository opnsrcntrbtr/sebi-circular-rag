...
## Corpus Check
- 231 files · ~212,998 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2834 nodes · 6009 edges · 180 communities (141 shown, 39 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 1478 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `afd3b2b8`
...
1. `Chunk` - 131 edges
2. `RAGPipeline` - 65 edges
3. `ExtractiveStubGenerator` - 59 edges
4. `HashEmbedder` - 50 edges
5. `hierarchical_chunk()` - 45 edges
6. `CircularMeta` - 43 edges
7. `load_golden()` - 37 edges
8. `build_lineage()` - 37 edges
9. `Lineage` - 35 edges
10. `answer_with_abstention()` - 34 edges
...
Cohesion: 0.17
Nodes (25): Re-score `pool` with a rewritten query when `reranked` is below `floor`.      Re, Fixed rewrite, for tests and for replaying a preregistered rewrite., rescue_pool(), StaticQueryRewriter, _chunk(), _EchoGenerator, _KeywordReranker, _pipeline() (+17 more)

### Community 8 - "eval_harness.py"
Cohesion: 0.05
Nodes (55): Protocol, Ground truth: what do the 4 CE_MISMATCH rows actually DO in production?  The 202, Preregistered cohort measurement for the CE paraphrase rescue.  Spec: `docs/supe, What does the 0.05 cross-encoder score floor actually catch?  `Settings.abstain_, cited_docs(), metrics(), Capture-once margin sweep for B' selective citations.  One pipeline pass over th, Margin sweep for B' selective citations on the golden_v7 adjudicated set.  One m (+47 more)
...
Cohesion: 0.05
Nodes (73): _doc(), _aggregate(), eligible(), main(), _measure(), phase_generate(), phase_report(), B' citation-scorer cohort measurement: control (bge, pointwise) vs J1 (jina, lis (+65 more)
...
Cohesion: 0.13
Nodes (21): load_circulars(), Path, Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., hierarchical_chunk(), _paragraphs(), Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, Document -> section -> paragraph chunks with stable IDs.      A "section" is det, _pipeline() (+13 more)
...
Cohesion: 0.08
Nodes (51): BaseModel, eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the roadm, main(), main() (+43 more)

### Community 22 - "Settings"
Cohesion: 0.06
Nodes (46): is_master(), main(), Is the eval set measuring retrieval, or measuring its own construction?  Read-on, main(), P0 prep: price a larger MLX generator before committing to the R0 upgrade.  Answ, rss_gb(), build_screen(), main() (+38 more)
...
Cohesion: 0.29
Nodes (7): log(), run(), One scoring path shared by `eval_json.py` (which measures) and `derive_threshold, Score one golden row through the production-shaped pipeline.      Returns per-ro, Per-row records -> metric -> score vector, skipping rows where the     metric wa, score_row(), vectors()
...
Cohesion: 0.11
Nodes (16): FastAPI, _citation_meta(), create_app(), _offline_pipeline(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit, metadata, /ready should trigger pipeline build and return ready=true., test_auth_required_when_key_set(), test_bge_fp16_encode_is_normalized() (+8 more)

### Community 31 - "HybridRetriever"
Cohesion: 0.08
Nodes (20): Embedder, ndarray, _tokens(), DenseIndex, _doc_checksum(), ndarray, F3 (ADR-001): encode only new/changed documents; reuse cached         embedding, Deterministic per-document checksum over its (enriched) chunk texts —     captur (+12 more)
...
Cohesion: 0.12
Nodes (35): Build a lightweight pipeline for --smoke mode.      Uses a stub retriever (no FA, smoke_pipeline(), answer_with_abstention(), ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., _chunk(), The gate must measure the context window, not just the fusion list.  `pipeline.q, An abstention still had a context window; measuring retrieval delivery     must (+27 more)
...
Cohesion: 0.09
Nodes (38): HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, LexicalReranker, Deterministic query-coverage reranker (test/fallback).      Score = fraction of, CircularMeta, _CannedGenerator, _distinct_pipeline(), _slow_pipeline() (+30 more)
...
Cohesion: 0.08
Nodes (23): _judge_prompt(), _judge_prompt_identify(), parse_excerpt_choice(), parse_yes_no(), Callable compatible with select_citations' scorer.rerank() signature.      Wraps, v2 protocol: closed-set identification instead of yes/no judgment.     Naming wh, True iff the reply names a valid excerpt number. 'none' or anything     unparsea, First yes/no in the reply; unparseable fails OPEN (grounded=True) so the     gat (+15 more)

### Community 61 - "Lineage"
Cohesion: 0.06
Nodes (58): main(), R3 §3.1 — mine cross-reference (A cites B) candidate pairs.  Spec: docs/superpow, contexts_for(), annotate_corpus(), build_lineage(), _currency(), demote_superseded(), detect_relations() (+50 more)
...
Cohesion: 0.16
Nodes (19): _compute_kwargs(), Resolve device/fp16/batch for the torch embedder + reranker., _keep(), load_circulars_from_hf(), load_corpus_records_from_hf(), load_hf_rows(), _meta_from_row(), HF-Hub corpus loading for the Hugging Face Spaces demo (CPU path).  Loads the pu (+11 more)
...
### Community 126 - "main"
Cohesion: 0.33
Nodes (4): _extracts(), MLXQueryRewriter, Paraphrase rescue for the cross-encoder score floor.  Preregistered in `docs/sup, Local MLX-LM rewriter. Greedy decoding -> deterministic.
...
### Community 128 - "is_degenerate"
Cohesion: 0.29
Nodes (7): is_degenerate(), True when `rewritten` is unusable and the rescue should be abandoned.      Degen, test_empty_rewrite_is_degenerate(), test_overlong_rewrite_is_degenerate(), test_plausible_rewrite_is_not_degenerate(), test_rewrite_at_the_word_limit_is_accepted(), test_unchanged_rewrite_is_degenerate()
...
### Community 131 - "reranker_interaction_check.py"
Cohesion: 0.47
...
### Community 159 - "test_persistence.py"
Cohesion: 0.67
...
### Community 162 - "main"
Cohesion: 1.00
Nodes (3): RAGPipeline, main(), smoke_pipeline()
...
- **Why does `Chunk` connect `SubjectSimJudge` to `TestPerQueryRecall`, `Frame`, `test_paraphrase_rescue.py`, `eval_harness.py`, `attribution.py`, `SpladeIndex`, `test_selective_citations.py`, `RAGPipeline`, `test_attribution.py`, `context_headers.py`, `load_golden`, `Settings`, `.query`, `HybridRetriever`, `ExtractiveStubGenerator`, `benchmark.py`, `test_rerank_jina_v3.py`, `_FakeRetriever`, `LexicalReranker`, `validate_golden_v7`, `Lineage`, `test_pipeline.py`, `demote_superseded`, `build_spaces_pipeline`, `answer_with_abstention`, `Qwen3MLXReranker`, `resolve_chunk_spans`, `parse_meta`, `main`, `test_injection.py`, `main`, `main`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `load_golden` to `TestPerQueryRecall`, `Frame`, `test_paraphrase_rescue.py`, `eval_harness.py`, `test_spaces.py`, `lineage.py`, `.query`, `HybridRetriever`, `test_api.py`, `ExtractiveStubGenerator`, `benchmark.py`, `_FakeRetriever`, `LexicalReranker`, `SubjectSimJudge`, `Lineage`, `measure_supersession_precision`, `resolve_chunk_spans`, `test_eval_harness_v7.py`, `main`, `Embedder`, `stats.py`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `TestRegistry`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `load_golden` to `gemini_adjudicate.py`, `adjudicate_draft.py`, `test_golden_v7_packet.py`, `Frame`, `benchmark.py`, `agreement.py`, `eval_harness.py`, `test_selective_citations.py`, `local_adjudicate.py`, `backfill_escalations.py`, `main`, `seed_v7.py`, `test_conformal.py`, `Settings`?**
...
- **Are the 46 inferred relationships involving `HashEmbedder` (e.g. with `_CannedGenerator` and `_distinct_pipeline()`) actually correct?**
  _`HashEmbedder` has 46 INFERRED edges - model-reasoned connections that need verification._