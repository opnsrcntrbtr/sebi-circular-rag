...
## Corpus Check
- 227 files · ~210,672 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2804 nodes · 5979 edges · 168 communities (134 shown, 34 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 1487 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `549e3e96`
...
1. `Chunk` - 131 edges
2. `RAGPipeline` - 67 edges
3. `ExtractiveStubGenerator` - 61 edges
4. `HashEmbedder` - 51 edges
5. `hierarchical_chunk()` - 46 edges
6. `CircularMeta` - 44 edges
7. `load_golden()` - 38 edges
8. `build_lineage()` - 38 edges
9. `BGEM3Embedder` - 35 edges
10. `Lineage` - 35 edges
...
### Community 3 - "Chunk"
Cohesion: 0.12
Nodes (30): BaseModel, _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, RegulationRef, RegulationSuccessor (+22 more)
...
Cohesion: 0.09
Nodes (39): _extracts(), is_degenerate(), MLXQueryRewriter, Paraphrase rescue for the cross-encoder score floor.  Preregistered in `docs/sup, Re-score `pool` with a rewritten query when `reranked` is below `floor`.      Re, Fixed rewrite, for tests and for replaying a preregistered rewrite., True when `rewritten` is unusable and the rescue should be abandoned.      Degen, Local MLX-LM rewriter. Greedy decoding -> deterministic. (+31 more)

### Community 8 - "eval_harness.py"
Cohesion: 0.13
Nodes (13): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production?  The 202, Preregistered cohort measurement for the CE paraphrase rescue.  Spec: `docs/supe, What does the 0.05 cross-encoder score floor actually catch?  `Settings.abstain_, Build the dense+sparse index once and persist it (run after corpus changes)., CE_MISMATCH false-abstention investigation (score-floor-diagnostic 2026-08-18)., Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the per, False positive check — hybrid gate over abstain=True rows.  Runs the pipeline ov, Hybrid gate sweep — preregistered analysis (2026-08-13).  Runs the pipeline over (+5 more)
...
Cohesion: 0.06
Nodes (50): classify_answer(), classify_query(), _doc(), load_run(), main(), Path, Classify golden/probe queries against a TREC runfile (throwaway research).  Clas, Answer-level classification: a candidate chunk qualifies if it contains     any (+42 more)

### Community 13 - "RAGPipeline"
Cohesion: 0.07
Nodes (53): load_circulars(), Path, CircularMeta, hierarchical_chunk(), _paragraphs(), Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, Document -> section -> paragraph chunks with stable IDs.      A "section" is det, _pipeline() (+45 more)
...
Cohesion: 0.11
Nodes (22): entailment_index(), NLIAttributionScorer, NLI attribution scoring for B' citation selection.  B' asks "does this context s, Index of the entailment class in a model's label map.      Read from the checkpo, Scores each context by P(entailment) of the answer given that context.      Impl, Wrap an already-constructed cross-encoder (also the test seam)., _softmax(), _chunk() (+14 more)
...
Cohesion: 0.10
Nodes (37): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the roadm, main(), main(), main() (+29 more)

### Community 22 - "Settings"
Cohesion: 0.06
Nodes (45): is_master(), main(), Is the eval set measuring retrieval, or measuring its own construction?  Read-on, call(), main(), _norm(), Does answering a golden_v7 row require a circular the corpus does not hold?  Spe, Uppercase, strip all whitespace — so 'CIR/MIRSD/5/ 2013' matches     'CIR/MIRSD/ (+37 more)
...
Cohesion: 0.13
Nodes (16): get_pipeline(), Cache one pipeline per mode; both share retriever/reranker/lineage., run(), Build a lightweight pipeline for --smoke mode.      Uses a stub retriever (no FA, smoke_pipeline(), Score one golden row through the production-shaped pipeline.      Returns per-ro, score_row(), eval_generator_for() (+8 more)
...
Cohesion: 0.20
Nodes (9): load_golden_asof(), Path, _lin_chain(), P4b: as-of golden evaluation runner tests (offline)., test_load_golden_asof_has_both_modes(), test_run_selector_cases_pass_and_fail(), test_summarize_all_pass_pins_upper_bound(), test_summarize_reports_a_clopper_pearson_interval() (+1 more)
...
Cohesion: 0.19
Nodes (23): beir_corpus_rows(), beir_query_rows(), build_golden_v6(), dir_fingerprint(), enrich_golden_item(), export_beir(), git_commit(), Any (+15 more)
...
Cohesion: 0.29
Nodes (4): log(), Margin sweep for B' selective citations on the golden_v7 adjudicated set.  One m, Emit one JSON line of retrieval/citation/abstention metrics using the persisted, One scoring path shared by `eval_json.py` (which measures) and `derive_threshold
...
Cohesion: 0.11
Nodes (17): main(), Create the enriched golden_v6 benchmark seed from frozen golden_v5.  This does n, BenchmarkIssue, per_query_recall(), Per-query recall@k at circular level, matching `run_retrieval_benchmark`.      A, validate_golden(), Ten chunks of one circular must not crowd the cutoff: the k applies         to u, Answerable-but-unjudged rows are excluded from metrics, never scored 0.  golden_ (+9 more)
...
Cohesion: 0.11
Nodes (28): smoke_pipeline(), assemble_pool(), Candidate pools for chunk-label judging (spec §6). TREC-style pooling: union of, TREC-style pool: gold-doc literal matches lead, then round-robin over     [reran, HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, LexicalReranker, Deterministic query-coverage reranker (test/fallback).      Score = fraction of (+20 more)
...
Cohesion: 0.10
Nodes (13): Protocol, Generator, _grounded_prompt(), Judge, F4 (ADR-001): retrieved text is explicitly delimited as quoted DATA and     the, QueryRewriter, Rewrites a lay-vocabulary query into statutory vocabulary.      Returns None whe, _LineageAwareReranker (+5 more)
...
### Community 136 - "TestPerQueryRecall"
Cohesion: 0.12
Nodes (17): Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Answer, faithfulness(), Generation with a hard abstention gate (D5).  If the top reranked score is below, Check that every circular id the answer cites (in square brackets) was     actua, End-to-end wiring: segment -> hybrid retrieve -> rerank -> generate/abstain., Stage-2 reranking (mandatory, D4). Cross-encoder in production; a deterministic (+9 more)
...
### Community 161 - "test_settings_citation_scorer_enabled_true"
Cohesion: 0.19
Nodes (15): cohen_kappa(), Categorical Cohen's kappa over paired labels (row-aligned). Each raw     element, _min_agreement_fixture(), Offline tests for golden-v7 agreement/promotion (spec 2026-07-23 sec 7): Cohen's, The kappa base-rate paradox: one label dominates, raw agreement is high,     yet, _same_provision_fixture(), test_claude_accuracy_ci_returns_exact_and_provision(), test_cohen_kappa_both_constant_and_identical_is_one() (+7 more)

### Community 179 - "discover_new.py"
Cohesion: 0.12
Nodes (10): Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped to, Emit one JSON line listing SEBI circulars newer than previously seen. Uses a sta, ADR-002 follow-up: compare the production subject-sim gate against the SECTION-A, _currency(), mc_topic(), P2 — cross-document supersession resolution.  Classifies each circular's referen, Normalised topic of a 'Master Circular for/on <TOPIC>' title, else None.      Us, Map any cited circular that is superseded -> the circular(s) superseding it. (+2 more)
...
- **Why does `Chunk` connect `SubjectSimJudge` to `Chunk`, `Frame`, `test_paraphrase_rescue.py`, `TestPerQueryRecall`, `eval_harness.py`, `test_golden_v7_gate.py`, `test_selective_citations.py`, `SpladeIndex`, `RAGPipeline`, `test_attribution.py`, `context_headers.py`, `load_golden`, `Settings`, `test_spaces.py`, `HybridRetriever`, `ExtractiveStubGenerator`, `benchmark.py`, `test_rerank_jina_v3.py`, `per_query_recall`, `LexicalReranker`, `validate_golden_v7`, `Lineage`, `test_pipeline.py`, `demote_superseded`, `build_spaces_pipeline`, `Qwen3MLXReranker`, `test_segment.py`, `resolve_chunk_spans`, `_confirms_claude`, `read_trec_run`, `main`, `test_injection.py`, `main`, `test_benchmark.py`?**
  _High betweenness centrality (0.152) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `Chunk` to `Frame`, `test_paraphrase_rescue.py`, `TestPerQueryRecall`, `RAGPipeline`, `load_golden`, `test_spaces.py`, `test_api.py`, `HybridRetriever`, `benchmark.py`, `per_query_recall`, `LexicalReranker`, `SubjectSimJudge`, `build_spaces_pipeline`, `measure_supersession_precision`, `test_eval_harness_v7.py`, `read_trec_run`, `main`, `Embedder`, `stats.py`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `TestRegistry`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `load_circulars()` connect `RAGPipeline` to `scrape_sebi.py`, `Chunk`, `test_golden_v7_packet.py`, `TestPerQueryRecall`, `SpladeIndex`, `backfill_escalations.py`, `test_segment.py`, `context_headers.py`, `load_golden`, `derive_validity`, `SubjectSimJudge`, `test_api.py`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 66 inferred relationships involving `Chunk` (e.g. with `NLIAttributionScorer` and `BenchmarkIssue`) actually correct?**
...
- **Are the 55 inferred relationships involving `ExtractiveStubGenerator` (e.g. with `get_pipeline()` and `run()`) actually correct?**
  _`ExtractiveStubGenerator` has 55 INFERRED edges - model-reasoned connections that need verification._
