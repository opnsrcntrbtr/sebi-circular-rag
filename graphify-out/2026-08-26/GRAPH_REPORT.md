...
## Corpus Check
- 224 files · ~209,240 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2798 nodes · 5889 edges · 187 communities (147 shown, 40 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 606 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8a3532ce`
...
Cohesion: 0.07
Nodes (35): call(), main(), _norm(), Does answering a golden_v7 row require a circular the corpus does not hold?…, Uppercase, strip all whitespace — so 'CIR/MIRSD/5/ 2013' matches…, Returns (answer, reasoning). The judge is a reasoning model: the oMLX API…, windows(), main() (+27 more)
...
Cohesion: 0.15
Nodes (28): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design…, main(), main(), main(), ADR-004 adoption: calibrate abstain_threshold for jina-reranker-v3-mlx's score…, main() (+20 more)
...
### Community 37 - "api.py"
Cohesion: 0.16
Nodes (13): BaseModel, CitationMeta, _compute_kwargs(), QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline. Run (real stack; loads the…, Resolve device/fp16/batch for the torch embedder + reranker., RegulationRef (+5 more)
...
Cohesion: 0.11
Nodes (16): Generator, Judge, Protocol, ADOPTED gate (eval_gate round 3): deterministic groundedness signal — max…, Max cosine(query, doc subject line) over contexts — the primary gate signal,…, Max cosine(query, section heading) over contexts — the second tier., SubjectSimJudge, JinaMLXReranker (+8 more)

### Community 61 - "Lineage"
Cohesion: 0.07
Nodes (50): contexts_for(), annotate_corpus(), build_lineage(), _currency(), demote_superseded(), detect_relations(), Lineage, Path (+42 more)
...
Cohesion: 0.13
Nodes (15): main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), What actually makes a context window large: chunk size, or chunk count? Read-…, main(), P0 prep: price a larger MLX generator before committing to the R0 upgrade.…, rss_gb(), _is_parseable() (+7 more)
...
### Community 162 - "test_settings_citation_scorer_enabled_false"
Cohesion: 0.47
...
### Community 182 - "reg_display_name"
Cohesion: 0.50
...
## Knowledge Gaps
- **62 isolated node(s):** `checks.sh script`, `measure.sh script`, `HF_HUB_DISABLE_XET`, `OMP_NUM_THREADS`, `PYTHONPATH` (+57 more)
...
- **Why does `Chunk` connect `SubjectSimJudge` to `TestRetrievalBenchmarkNdcg`, `Chunk`, `test_paraphrase_rescue.py`, `eval_harness.py`, `SpladeIndex`, `test_golden_v7_gate.py`, `test_selective_citations.py`, `RAGPipeline`, `test_attribution.py`, `context_headers.py`, `load_golden`, `test_spaces.py`, `load_circulars`, `HybridRetriever`, `ExtractiveStubGenerator`, `api.py`, `benchmark.py`, `test_rerank_jina_v3.py`, `.query`, `test_expand.py`, `OllamaGenerator`, `validate_golden_v7`, `Lineage`, `demote_superseded`, `build_spaces_pipeline`, `answer_with_abstention`, `test_hyde.py`, `Qwen3MLXReranker`, `test_segment.py`, `resolve_chunk_spans`, `sweep_rrf_k.py`, `read_trec_run`, `main`, `test_injection.py`, `main`, `test_benchmark.py`?**
  _High betweenness centrality (0.138) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `TestRetrievalBenchmarkNdcg`, `measure_context_precision`, `TestPerQueryRecall`, `test_paraphrase_rescue.py`, `eval_harness.py`, `load_golden`, `lineage.py`, `test_api.py`, `api.py`, `benchmark.py`, `settings.py`, `.query`, `per_query_recall`, `SubjectSimJudge`, `Lineage`, `test_pipeline.py`, `build_spaces_pipeline`, `measure_supersession_precision`, `test_trec_parity.py`, `test_eval_harness_v7.py`, `run_judge`, `Embedder`, `measure_mrr`, `measure_parsing_latency`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `TestRegistry`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `sebi_rag/eval_asof.py` connect `lineage.py` to `clopper_pearson_ci`, `eval_harness.py`, `RAGPipeline`, `paired_delta`, `build_report`, `Lineage`?**
