...
## Summary
- 2939 nodes · 6228 edges · 189 communities (147 shown, 42 thin omitted)
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 1540 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3aad44f3`
...
8. `build_lineage()` - 39 edges
9. `load_golden()` - 38 edges
10. `Lineage` - 35 edges
...
Cohesion: 0.10
Nodes (17): FastAPI, create_app(), _offline_pipeline(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit, metadata, /ready should trigger pipeline build and return ready=true., _slow_pipeline(), _SlowGenerator, test_auth_required_when_key_set() (+9 more)
...
Cohesion: 0.06
Nodes (55): Random, build_screen(), T-Screen: does the generator follow the citation instruction at all?  Spec: docs, 50 rows stratified proportionally to golden_v7's eight strata., _apportion(), ingest_packet(), _ingest_to_votes(), main() (+47 more)
...
Cohesion: 0.10
Nodes (26): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production?  The 202, Preregistered cohort measurement for the CE paraphrase rescue.  Spec: `docs/supe, What does the 0.05 cross-encoder score floor actually catch?  `Settings.abstain_, cited_docs(), metrics(), Capture-once margin sweep for B' selective citations.  One pipeline pass over th, Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use -- (+18 more)
...
### Community 13 - "api.py"
Cohesion: 0.15
Nodes (23): BaseModel, _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, RegulationRef, RegulationSuccessor (+15 more)
...
Cohesion: 0.14
Nodes (18): entailment_index(), Index of the entailment class in a model's label map.      Read from the checkpo, Wrap an already-constructed cross-encoder (also the test seam)., _chunk(), _FakeNLI, NLI attribution scorer for B' citation selection.  B' needs to know whether a co, Failing loudly beats scoring on an arbitrary class., Stands in for the cross-encoder: maps context text -> 3 class logits. (+10 more)
...
Cohesion: 0.09
Nodes (40): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the roadm, main(), main(), main() (+32 more)
...
### Community 24 - ".query"
Cohesion: 0.19
Nodes (18): _assert_fixed_tail(), convert_run_dir(), main(), Path, Back-convert archived runfiles into standards-compliant TREC artifacts.  The arc, Trailing field of the first line; also the whitespace precondition check., read_trec_run assumes qid and tag carry no whitespace. Verify per line., Write run.chunk.trec, run.doc.trec and docids.tsv for one archived run. (+10 more)
...
Cohesion: 0.13
Nodes (14): Standards-compliant TREC run and qrels emission.  The archived runfiles are not, per_query_recall(), Per-query recall@k at circular level, matching `run_retrieval_benchmark`.      A, _doc(), _eval_item(), _unique(), Ten chunks of one circular must not crowd the cutoff: the k applies         to u, Answerable-but-unjudged rows are excluded from metrics, never scored 0.  golden_ (+6 more)

### Community 54 - "test_lineage.py"
Cohesion: 0.09
Nodes (37): contexts_for(), demote_superseded(), detect_relations(), detect_relations_ex(), Down-weight reranked (chunk, score) pairs from superseded circulars and     re-s, Like detect_relations, but returns dict records with evidence spans., Return (relation, referenced_circular) for each distinct reference., _window() (+29 more)

### Community 55 - "regulations.py"
Cohesion: 0.17
Nodes (16): Rankings, Path, Reverse map `docid -> full chunk id`, so nothing is lost., Valid 6-field TREC run at chunk granularity., Valid 6-field TREC run collapsed to circular level.      Keeps each circular onc, write_docids(), _write_lines(), write_run_chunk() (+8 more)
...
Cohesion: 0.14
Nodes (9): Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, # NOTE: We intentionally DO NOT set carry here — flush() already, Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, P1 evaluation-harness test (offline).  Loads the real seed corpus (data/corpus/c, test_eval_harness_metric_suite(), _ollama_up() (+1 more)
...
Cohesion: 0.24
Nodes (9): log(), Margin sweep for B' selective citations on the golden_v7 adjudicated set.  One m, run(), One scoring path shared by `eval_json.py` (which measures) and `derive_threshold, Score one golden row through the production-shaped pipeline.      Returns per-ro, Per-row records -> metric -> score vector, skipping rows where the     metric wa, score_row(), vectors() (+1 more)

### Community 80 - "test_eval_harness_v7.py"
Cohesion: 0.30
Nodes (14): _aggregate(), EvalReport, _mean(), report_dict(), run_eval(), _pipeline(), Offline harness tests for v7 metrics: as_of passthrough, must_not_cite, chunk-le, _row() (+6 more)
...
Cohesion: 0.08
Nodes (41): hierarchical_chunk(), Document -> section -> paragraph chunks with stable IDs.      A "section" is det, _build_chunks(), _build_pipeline(), _FixedReranker, _master_reissue_pipeline(), Minimal end-to-end test of the SEBI RAG pipeline.  Runs fully offline (HashEmbed, Offline pipeline whose single circular rests on a repealed regulation. (+33 more)
...
Cohesion: 0.22
Nodes (8): main(), Create the enriched golden_v6 benchmark seed from frozen golden_v5.  This does n, BenchmarkIssue, _span_resolution_issues(), validate_golden(), v7-ls-038/039/040 are answerable but unjudged; they carry     expected_citation_, test_the_three_known_unjudged_rows_are_warnings_only(), test_validate_golden_still_errors_on_real_corruption()
...
Cohesion: 0.08
Nodes (45): Build a lightweight pipeline for --smoke mode.      Uses a stub retriever (no FA, smoke_pipeline(), smoke_pipeline(), assemble_pool(), Candidate pools for chunk-label judging (spec §6). TREC-style pooling: union of, TREC-style pool: gold-doc literal matches lead, then round-robin over     [reran, load_circulars(), Path (+37 more)
...
Cohesion: 0.46
Nodes (7): _chunk(), The gate must measure the context window, not just the fusion list.  `pipeline.q, An abstention still had a context window; measuring retrieval delivery     must, _reranked(), test_answer_records_the_context_ids_it_used(), test_context_ids_populated_even_when_abstaining(), test_context_ids_respect_top_k()
...
- **Why does `Chunk` connect `Chunk` to `test_context_headers.py`, `Frame`, `test_paraphrase_rescue.py`, `generate.py`, `test_context_recall.py`, `SpladeIndex`, `test_selective_citations.py`, `api.py`, `test_attribution.py`, `HybridRetriever`, `test_rerank_set_encoder.py`, `answer_with_abstention`, `consolidation_edges`, `benchmark.py`, `resolve_chunk_spans`, `validate_golden_v7`, `test_lineage.py`, `test_rerank_jina_v3.py`, `test_spaces.py`, `segment.py`, `test_certainty.py`, `Qwen3MLXReranker`, `build_spaces_pipeline`, `test_golden_v7_pool.py`, `test_pipeline.py`, `.build_incremental`, `RAGPipeline`, `test_hyde.py`, `test_injection.py`, `context_headers.py`, `main`, `Embedder`, `.grounded`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `api.py` to `test_api.py`, `Frame`, `test_paraphrase_rescue.py`, `generate.py`, `HybridRetriever`, `sebi_rag/eval_asof.py`, `run_all_metrics`, `Chunk`, `benchmark.py`, `_unique`, `measure_parsing_latency`, `hybrid_gate_sweep.py`, `measure_supersession_precision`, `MeasureResult`, `build_spaces_pipeline`, `sweep_citation_margin.py`, `test_eval_harness_v7.py`, `test_pipeline.py`, `.build_incremental`, `RAGPipeline`, `measure.py`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `Embedder`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `hierarchical_chunk()` connect `test_pipeline.py` to `segment.py`, `measure.py`, `test_api.py`, `resolve_chunk_spans`, `build_spaces_pipeline`, `test_eval_harness_v7.py`, `test_persistence.py`, `Chunk`, `HybridRetriever`, `measure_parsing_latency`, `Embedder`, `RAGPipeline`?**
