...
## Corpus Check
- 196 files · ~184,494 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2500 nodes · 5057 edges · 163 communities (128 shown, 35 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 300 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `25994bc6`
...
1. `Chunk` - 54 edges
2. `RAGPipeline` - 45 edges
3. `hierarchical_chunk()` - 44 edges
4. `CircularMeta` - 39 edges
5. `HashEmbedder` - 37 edges
6. `ExtractiveStubGenerator` - 36 edges
7. `build_lineage()` - 31 edges
8. `Lineage` - 30 edges
...
Cohesion: 0.20
Nodes (20): Any, Chunk, beir_corpus_rows(), beir_query_rows(), BenchmarkIssue, build_golden_v6(), chunks_by_doc(), enrich_golden_item() (+12 more)
...
Cohesion: 0.06
Nodes (36): contexts_for(), demote_superseded(), detect_relations(), detect_relations_ex(), Lineage, Path, Down-weight reranked (chunk, score) pairs from superseded circulars and re-…, Map any cited circular that is superseded -> the circular(s) superseding it.… (+28 more)
...
Cohesion: 0.10
Nodes (15): FastAPI, integration, create_app(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit,…, /ready should trigger pipeline build and return ready=true., test_auth_required_when_key_set(), test_bge_fp16_encode_is_normalized(), test_citation_meta_defaults_when_circular_absent_from_index() (+7 more)
...
Cohesion: 0.10
Nodes (34): _chunk(), _FakeReranker, Tests for B' selective citations: select_citations() and its integration., When citation_scorer_enabled=True, Settings loads a non-None scorer., When citation_scorer_enabled=False, Settings loads scorer disabled., The backend choice must go through the same single decision point as     the ena, Deterministic scorer: returns preset answer-relevance scores, sorted desc., Looser margin (0.45) keeps more contexts than tight margin (0.35). (+26 more)
...
Cohesion: 0.15
Nodes (7): Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped…, Build the dense+sparse index once and persist it (run after corpus changes).…, Emit one JSON line listing SEBI circulars newer than previously seen. Uses a…, _currency(), mc_topic(), P2 — cross-document supersession resolution. Classifies each circular's…, Normalised topic of a 'Master Circular for/on <TOPIC>' title, else None. Used…
...
Cohesion: 0.14
Nodes (31): BaseModel, Build the full pipeline with real models., real_pipeline(), main(), main(), main(), build_default_pipeline(), _citation_meta() (+23 more)
...
### Community 49 - "_doc"
Cohesion: 0.12
Nodes (30): Path, corpus.load_circulars edge-case coverage.  load_circulars reads a JSONL corpus f, Provided optional fields are passed through to CircularMeta., Multiple records produce multiple chunks., Blank lines between records are silently skipped., Malformed JSON raises ValueError (json.loads default)., load_circulars accepts both str and Path., load_circulars accepts a pathlib.Path. (+22 more)
...
Cohesion: 0.16
Nodes (18): Path, Which golden set gates CI, and whether its adjudicated subset clears the…, Resolution order: explicit SEBI_RAG_GOLDEN override, then the armed v7 gate,…, select_golden(), _gate_file(), Offline tests for the golden-v7 CI gate flip (spec 2026-07-23 sec 8). The gate…, derive_floors reads its vectors from score.vectors(); a metric absent there can…, floors_ok fails closed on a metric the report does not carry, so a floor name… (+10 more)
...
Cohesion: 0.11
Nodes (16): faithfulness(), _judge_prompt(), _judge_prompt_identify(), MLXJudge, parse_excerpt_choice(), parse_yes_no(), Generation with a hard abstention gate (D5).  If the top reranked score is below, v2 protocol: closed-set identification instead of yes/no judgment.     Naming wh (+8 more)
...
Cohesion: 0.17
Nodes (13): dir_fingerprint(), git_commit(), Path, Parse a runfile written by `write_trec_run` back into {qid: [(doc, score)]}., Write optional research-only judge outputs beside, never into, gold labels., Stable digest over files in a directory tree., read_trec_run(), run_metadata() (+5 more)
...
Cohesion: 0.11
Nodes (21): RAGPipeline, Benchmark MLX generators on the golden set: faithfulness, groundedness,…, main(), Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, smoke_pipeline(), Calibrate top_k and the abstention threshold against the citation-precision…, scripts/eval_asof.py, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the… (+13 more)
...
Cohesion: 0.21
Nodes (12): HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable…, _chunk(), test_expand_sparse_off_routes_raw_query_to_sparse_leg(), test_retrieve_dense_leg_keeps_raw_query(), test_retrieve_routes_expanded_query_to_sparse_leg(), One gold doc with `n` chunks that ALL contain the word "broker", so a…, _saturating_retriever() (+4 more)

### Community 78 - "_is_non_sebi_domain"
Cohesion: 0.08
Nodes (17): The non-SEBI domain filter must match words, not substrings.  Shipped 2026-07-30, Any single-token keyword <= 5 chars is a substring hazard. Embedding it     insi, Query mentioning both SEBI and RBI should NOT abstain — SEBI intent wins., Empty query should not trigger the non-SEBI filter., FEMA keyword in a SEBI context should NOT abstain — SEBI intent wins., The exact query that exposed the bug., Guard against false positives on cross-domain SEBI questions., Documented as a keyword since 2026-07-30 but never actually present in     _NON_ (+9 more)
...
Cohesion: 0.24
Nodes (14): Answer, answer_with_abstention(), _is_non_sebi_domain(), Return True if the query clearly targets a non-SEBI regulator's domain.      Cas, Context ids the answer rests on. Scores each context's answer-relevance     via, select_citations(), _chunk(), Offline tests for the ADR-002 certainty architecture: abstention reasons, confid (+6 more)
...
Cohesion: 0.10
Nodes (28): load_circulars(), Path, Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., measure_parsing_latency(), Measure PDF ingestion throughput (chars/sec, ms/PDF). Samples 20 PDFs…, hierarchical_chunk(), _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs. Minimal,… (+20 more)
...
Cohesion: 0.12
Nodes (31): Embedder, Lineage, Reranker, Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., build_lineage() (+23 more)
...
Cohesion: 0.16
Nodes (7): main(), metrics_to_markdown(), Format results as a markdown table., Unit tests for sebi_rag.measure — automated metric collection., Test with a dummy PDF file — should not crash., TestCLI, TestParsingLatency

### Community 136 - "resolve_stems"
Cohesion: 0.33
...
### Community 140 - "build_chunk_rows"
Cohesion: 0.27
...
### Community 150 - "build_citation_pairs"
Cohesion: 0.29
...
### Community 154 - "TestPerQueryRecall"
Cohesion: 0.38
Nodes (4): IncomparableFramesError, Raised when a paired comparison spans two frames., Ten chunks of one circular must not crowd the cutoff: the k applies to unique…, TestPerQueryRecall
...
## Knowledge Gaps
- **50 isolated node(s):** `dev.sh script`, `HF_HUB_DISABLE_XET`, `OMP_NUM_THREADS`, `PYTHONPATH`, `PYTORCH_ENABLE_MPS_FALLBACK` (+45 more)
...
- **Why does `Chunk` connect `consolidation_edges` to `test_spaces.py`, `api_spaces.py`, `sweep_rrf_k.py`, `corpus.py`, `Lineage`, `SparseIndex`, `test_attribution.py`, `_grounded_prompt`, `test_annotation_adds_no_circular_meta_field`, `RAGPipeline`, `Chunk`, `.build`, `SpladeIndex`, `Benchmark Schema Validation`, `generate.py`, `test_incremental_index.py`, `Prompt Injection Security`, `test_persistence.py`, `test_persistence.py`, `hierarchical_chunk`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `.query`, `test_persistence.py`, `measure_supersession_precision`, `api_spaces.py`, `generate.py`, `Context Precision Metrics`, `measure.py`, `measure_mrr`, `write_trec_qrels`, `eval_harness.py`, `measure_retrieval_recall`, `hierarchical_chunk`, `measure_temporal_accuracy`, `Frame`, `API Service and Integration`, `build_citation_pairs`, `TestPerQueryRecall`, `sebi_rag/eval_asof.py`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `sebi_rag/eval_asof.py` connect `sebi_rag/eval_asof.py` to `settings.py`, `RAGPipeline`, `generate.py`, `Lineage`, `test_pipeline.py`, `clopper_pearson_ci`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Are the 18 inferred relationships involving `Chunk` (e.g. with `NLIAttributionScorer` and `HeaderGenerator`) actually correct?**
...
- **Are the 8 inferred relationships involving `HashEmbedder` (e.g. with `_CannedGenerator` and `_SlowGenerator`) actually correct?**
  _`HashEmbedder` has 8 INFERRED edges - model-reasoned connections that need verification._