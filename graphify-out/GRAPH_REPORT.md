...
### Community 0 - "Chunk"
Cohesion: 0.06
Nodes (25): Generator, Judge, MLXGenerator, Protocol, ADOPTED gate (eval_gate round 3): deterministic groundedness signal — max…, Max cosine(query, doc subject line) over contexts — the primary gate signal,…, Max cosine(query, section heading) over contexts — the second tier., Apple-Silicon-native generation via MLX-LM (D6 preferred runtime). Loads a… (+17 more)
...
Cohesion: 0.06
Nodes (61): eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design…, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), Phase 1 (systematic-debugging) evidence gathering for the 2026-09-03…, main() (+53 more)
...
Cohesion: 0.08
Nodes (59): Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), smoke_pipeline(), run_retrieval_benchmark(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable…, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required. (+51 more)
...
Cohesion: 0.07
Nodes (28): _grounded_prompt(), OllamaGenerator, F4 (ADR-001): retrieved text is explicitly delimited as quoted DATA and the…, Grounded generation via local Ollama (D6 canonical runtime option).…, ExternalSpaceGenerator, HFGenerator, HybridGenerator, CPU / remote generation for the Hugging Face Spaces demo. All classes implement… (+20 more)
...
Cohesion: 0.11
Nodes (29): Answer, answer_with_abstention(), _abstain(), faithfulness(), Check that every circular id the answer cites (in square brackets) was actually…, As-of exclusion or supersession demotion, applied to a reranked list. Extracted…, test_faithfulness_scoring(), _chunk() (+21 more)
...
Cohesion: 0.09
Nodes (31): _is_signoff_boilerplate(), _leaks_metadata(), mine_heading_section(), mine_subject_body(), First line matching the numbered-clause pattern -> (heading, rest). None if no…, _split_heading(), _FakeDense, _FakeDenseIndex (+23 more)
...
### Community 33 - "main"
Cohesion: 0.10
Nodes (33): _chunk_id(), _doc(), load_pre_expansion_ids(), main(), Path, W1.3 diagnostic (2026-09-03 architecture review): does the 730->1,490 corpus…, _unique(), main() (+25 more)
...
Cohesion: 0.10
Nodes (20): Embedder, ndarray, Protocol, _tokens(), DenseIndex, _doc_checksum(), _embedder_identity(), ndarray (+12 more)
...
### Community 43 - "write_dataset_cards"
Cohesion: 0.17
...
Cohesion: 0.11
Nodes (29): build_citation_pairs(), build_supersession_pairs(), Pure transform: corpus + lineage -> labeled circular pairs. label is…, Pure transform: corpus text -> citation-normalization rows. Mines in-body…, Every chunk's text is `"{doc_id} | {subject[:120]} | {section}\\n{body}"` -…, _strip_context_header(), cache_path(), call_omlx() (+21 more)

### Community 45 - "gemini_adjudicate.py"
Cohesion: 0.13
Nodes (19): _current_model(), _daily_quota_exhausted(), _parse_letter_choice(), _parse_reply(), _parse_yes_no(), _post_gemini(), External annotation slice: second-family LLM leg via the Gemini API (spec…, Letter-choice protocol: first non-blank line is the choice (comma or semicolon… (+11 more)
...
Cohesion: 0.19
Nodes (15): Same per-row deterministic shuffle as make_packet.py's write_packet:…, _shuffled_candidates(), _current_model(), main(), pilot(), _pilot_ids(), _post_local(), Path (+7 more)
...
### Community 52 - "_FakeReranker"
Cohesion: 0.18
Nodes (15): citation_scorer_for(), The single enable/disable AND backend decision for B'. Returns None when…, _FakeReranker, The backend choice must go through the same single decision point as the enable…, Deterministic scorer: returns preset answer-relevance scores, sorted desc., The `reranker` positional is the bge instance used by backend="reranker";…, test_citation_scorer_for_defaults_to_the_reranker_backend(), test_citation_scorer_for_disabled_beats_backend_choice() (+7 more)

### Community 53 - "test_selective_citations.py"
Cohesion: 0.15
Nodes (23): Context ids the answer rests on. Scores each context via `scorer`, keeps those…, select_citations(), _chunk(), Tests for B' selective citations: select_citations() and its integration., Looser margin (0.45) keeps more contexts than tight margin (0.35)., When citation scorer is None, all contexts are cited (legacy behavior)., Empty context list returns empty citation list., Measured 2026-08-12: on 206 rows where retrieval found every relevant doc, B'… (+15 more)

### Community 54 - "segment.py"
Cohesion: 0.07
Nodes (47): load_circulars(), Path, Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., hierarchical_chunk(), flush(), _is_table_row_candidate(), _is_table_row_filler(), _is_toc_row_candidate() (+39 more)
...
### Community 58 - "export_datasets.py"
Cohesion: 0.16
Nodes (29): build_eval_rows(), build_lineage_rows(), _config_entry(), _emit(), export_all(), export_chunks(), export_citation_normalization(), export_corpus() (+21 more)
...
### Community 60 - "acquire_missing_pdfs.py"
Cohesion: 0.20
...
### Community 64 - "test_certainty.py"
Cohesion: 0.39
...
### Community 65 - "test_reg_lineage.py"
Cohesion: 0.15
...
### Community 69 - "build_regulation_edges"
Cohesion: 0.22
...
### Community 76 - "main"
Cohesion: 0.09
Nodes (18): main(), _compute_kwargs(), _embed_kwargs(), _compute_kwargs() plus model_path - BGEM3Embedder-only, never spread into…, Resolve device/fp16/batch for the torch embedder + reranker. Shared verbatim…, HydeExpander, HyDE (Hypothetical Document Embeddings): query -> statutory passage. Part B of…, JinaMLXReranker (+10 more)
...
### Community 79 - "roundtrip_filter.py"
Cohesion: 0.29
...
Cohesion: 0.44
Nodes (11): run_eval(), _pipeline(), Offline harness tests for v7 metrics: as_of passthrough, must_not_cite, chunk-…, _row(), test_as_of_is_passed_to_pipeline(), query(), test_chunk_metrics_computed_for_span_rows(), test_gate_is_none_when_nothing_adjudicated() (+3 more)

### Community 83 - "test_lineage.py"
Cohesion: 0.06
Nodes (51): contexts_for(), annotate_corpus(), demote_superseded(), detect_relations(), detect_relations_ex(), Lineage, Path, Down-weight reranked (chunk, score) pairs from superseded circulars and re-… (+43 more)
...
Cohesion: 0.12
Nodes (18): expand_query(), Query-side lexical expansion for BM25 (intervention #2, glossary variant). SEBI…, Append statutory synonyms for lay tokens present in `query`. Deterministic and…, BM25 lexical index (bm25s)., Reciprocal Rank Fusion. Rank-only — sidesteps score-scale mismatch., rrf_fuse(), SparseIndex, Query-side lexical expansion (intervention #2, glossary variant).… (+10 more)

### Community 90 - "_parse_error_ids"
Cohesion: 0.20
Nodes (10): main(), _parse_error_ids(), Path, Scans the per-row cache for `ids` and returns the ones flagged parse_error:…, Rerun-safety for votes.jsonl itself (plan Task 10 decision #7): drops every…, _replace_annotator_votes(), Defensive: an id that was never adjudicated (no cache file at all) is not…, test_parse_error_ids_skips_ids_with_no_cache_file() (+2 more)

### Community 91 - "WarrantJudge"
Cohesion: 0.20
...
Cohesion: 0.18
Nodes (16): Random, load_chunks_by_doc(), load_corpus_records(), load_minable_docs(), main(), mine_citation_context(), mine_lineage_pairs(), Path (+8 more)
...
Cohesion: 0.18
Nodes (4): Offline tests for the missing-PDF recovery logic (no network)., test_resolve_stems_matches_by_stem(), test_resolve_stems_survives_detail_fetch_error(), bad_fetch()
...
### Community 106 - "test_citation_scorer_for_warrant_max_tokens_defaults_unset"
Cohesion: 0.20
Nodes (7): warrant_model/warrant_shared default to None. Forwarding None explicitly would…, Omitted, not forwarded as None — warrant_scorer's own default applies., test_citation_scorer_for_forwards_warrant_max_tokens(), test_citation_scorer_for_selects_the_warrant_backend(), _fake_warrant_scorer(), test_citation_scorer_for_warrant_max_tokens_defaults_unset(), test_citation_scorer_for_warrant_omits_unset_kwargs()
...
### Community 110 - "resolve_chunk_spans"
Cohesion: 0.30
...
### Community 120 - "build_chunk_rows"
Cohesion: 0.40
...
### Community 143 - "relabel_repooled.py"
Cohesion: 0.43
...
### Community 154 - "test_integration_e2e.py"
Cohesion: 0.33
...
### Community 162 - "test_golden_v7_local.py"
Cohesion: 0.17
...
### Community 165 - "SetEncoderReranker"
Cohesion: 0.40
...
- **Why does `Chunk` connect `Chunk` to `test_injection.py`, `generate.py`, `test_golden_v7_gate.py`, `context_headers.py`, `test_paraphrase_rescue.py`, `test_benchmark.py`, `main`, `HybridRetriever`, `test_attribution.py`, `RAGPipeline`, `test_spaces.py`, `answer_with_abstention`, `test_spaces_app.py`, `SetEncoderReranker`, `Embedder`, `benchmark.py`, `warrant_scorer`, `test_selective_citations.py`, `segment.py`, `test_rerank_set_encoder.py`, `test_rerank_jina_v3.py`, `.grounded`, `test_certainty.py`, `SpladeIndex`, `main`, `validate_golden_v7`, `test_lineage.py`, `corpus_spaces.py`, `test_expand.py`, `WarrantJudge`, `Qwen3MLXReranker`, `resolve_chunk_spans`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `Chunk`, `generate.py`, `test_paraphrase_rescue.py`, `test_api.py`, `HybridRetriever`, `measure_mrr`, `measure_parsing_latency`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `answer_with_abstention`, `test_integration_e2e.py`, `measure_context_precision`, `Embedder`, `benchmark.py`, `_doc`, `sebi_rag/eval_asof.py`, `MeasureResult`, `main`, `test_eval_harness_v7.py`, `test_lineage.py`, `measure.py`, `hybrid_gate_sweep.py`, `measure_supersession_precision`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `HybridRetriever` to `generate.py`, `run_judge`, `Frame`, `main`, `relabel_repooled.py`, `seed_v7.py`, `test_finetune_eval_phase0.py`, `test_golden_v7_packet.py`, `main`, `main`, `benchmark.py`, `main`, `test_finetune_holdout.py`, `backfill_escalations.py`, `local_adjudicate.py`, `agreement.py`, `segment.py`, `_doc`, `MeasureResult`, `main`, `_parse_error_ids`, `remap_doc_ids.py`, `hybrid_gate_sweep.py`, `RuntimeError`, `phase_generate`, `phase_judge`?**
