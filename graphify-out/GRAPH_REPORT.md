...
## Corpus Check
- 90 files · ~47,844 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 962 nodes · 2092 edges · 47 communities (39 shown, 8 thin omitted)
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 442 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2046c4fd`
...
- `test_detect_relations_ex_evidence_and_extractor()` --calls--> `detect_relations_ex()`  [INFERRED]
...
Cohesion: 0.07
Nodes (55): Any, _doc(), auroc(), best_threshold(), evaluate(), F2 (ADR-001): benchmark rerankers on golden_v5 with cluster-separation metrics., P(pos_score > neg_score); ties count half. pos = answerable top-scores,     neg, Threshold maximising abstention accuracy: answer if score >= thr.     Returns (t (+47 more)
...
Cohesion: 0.17
Nodes (16): AsofCaseResult, load_golden_asof(), Path, As-of-date golden evaluation runner (P4b).  Two case modes drawn from eval/golde, run_pipeline_cases(), run_selector_cases(), summarize(), Lineage (+8 more)
...
Cohesion: 0.15
Nodes (19): hierarchical_chunk(), _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, Document -> section -> paragraph chunks with stable IDs.      A "section" is det, _chunks(), Index persistence round-trip (offline)., _body() (+11 more)
...
Cohesion: 0.13
Nodes (11): Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Calibrate top_k and the abstention threshold against the citation-precision sign, Embedder protocol + a deterministic test embedder + the real bge-m3 embedder.  T, LexicalReranker, Stage-2 reranking (mandatory, D4). Cross-encoder in production; a deterministic, Deterministic query-coverage reranker (test/fallback).      Score = fraction of, _HallucinatingGenerator, Faithfulness: catch answers that cite circulars not in the retrieved context. (+3 more)

### Community 20 - "test_gate.py"
Cohesion: 0.06
Nodes (51): Protocol, Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the per, Answer, answer_with_abstention(), faithfulness(), Generator, _grounded_prompt() (+43 more)
...
Cohesion: 0.20
Nodes (19): smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., CircularMeta, _offline_pipeline(), test_run_pipeline_cases_pass_and_avoid() (+11 more)
...
Cohesion: 0.23
Nodes (13): Pipeline builder for the Hugging Face Spaces demo (CPU-only, Linux).  Parallel t, _keep(), load_circulars_from_hf(), load_corpus_records_from_hf(), load_hf_rows(), _meta_from_row(), HF-Hub corpus loading for the Hugging Face Spaces demo (CPU path).  Loads the pu, One HF dataset config as plain dicts (network; cached by `datasets`). (+5 more)
...
Cohesion: 0.15
Nodes (19): build_lineage(), _currency(), _lin_chain(), P2 lineage / supersession resolution tests., test_build_lineage_edges_tiered(), test_build_lineage_inferred_master_topic_edge(), test_detect_relations_ex_evidence_and_extractor(), test_governing_on_before_family_exists() (+11 more)

### Community 31 - ".load"
Cohesion: 0.17
Nodes (18): BaseModel, main(), build_default_pipeline(), _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the (+10 more)
...
Cohesion: 0.10
Nodes (15): Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped to, Emit one JSON line listing SEBI circulars newer than previously seen. Uses a sta, Emit one JSON line of retrieval/citation/abstention metrics over golden_v5 (env, detect_relations(), detect_relations_ex(), mc_topic(), P2 — cross-document supersession resolution.  Classifies each circular's referen, Normalised topic of a 'Master Circular for/on <TOPIC>' title, else None.      Us (+7 more)
...
- **Why does `Chunk` connect `test_gate.py` to `Benchmark Infrastructure`, `Data Processing`, `Spaces CPU Pipeline`, `Lineage`, `Benchmark Scripts`, `test_integration_e2e.py`, `Embedder`, `answer_with_abstention`, `Chunk`, `Answer Generation`, `SubjectSimJudge`, `eval_gate.py`, `.load`?**
  _High betweenness centrality (0.153) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `Lineage` to `Benchmark Infrastructure`, `Embedder`, `Master Metadata`, `answer_with_abstention`, `test_gate.py`, `Answer Generation`, `SubjectSimJudge`, `.load`?**
