...
### Community 3 - "Chunk"
Cohesion: 0.12
Nodes (19): BaseModel, _citation_meta(), CitationMeta, _compute_kwargs(), QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline. Run (real stack; loads the…, Resolve device/fp16/batch for the torch embedder + reranker. (+11 more)
...
Cohesion: 0.09
Nodes (53): Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable…, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., RAGPipeline (+45 more)
...
Cohesion: 0.11
Nodes (35): integration, eligible(), main(), SPIKE — throwaway, not preregistered. Answers one question before any R6 design…, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), main() (+27 more)
...
Cohesion: 0.11
Nodes (12): FastAPI, create_app(), _CannedGenerator, FastAPI service tests (offline pipelines): endpoints, auth, rate limit,…, /ready should trigger pipeline build and return ready=true., _SlowGenerator, test_auth_required_when_key_set(), test_citation_meta_reports_superseded() (+4 more)
...
Cohesion: 0.09
Nodes (15): Generator, _grounded_prompt(), Judge, OllamaGenerator, Protocol, F4 (ADR-001): retrieved text is explicitly delimited as quoted DATA and the…, Grounded generation via local Ollama (D6 canonical runtime option).…, _extracts() (+7 more)

### Community 61 - "Lineage"
Cohesion: 0.08
Nodes (44): contexts_for(), annotate_corpus(), build_lineage(), _currency(), demote_superseded(), Lineage, mc_topic(), Path (+36 more)
...
Cohesion: 0.10
Nodes (19): mrr(), Minimal retrieval metrics (subset of docs/project_context.md section 7).…, recall_at_k(), measure_parsing_latency(), measure_supersession_precision(), Path, Automated metric collection for the SEBI Circular RAG pipeline. Six on-demand…, Measure PDF ingestion throughput (chars/sec, ms/PDF). Samples 20 PDFs… (+11 more)
...
Cohesion: 0.11
Nodes (23): Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., hierarchical_chunk(), _paragraphs(), Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, # NOTE: We intentionally DO NOT set carry here — flush() already, Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, Document -> section -> paragraph chunks with stable IDs.      A "section" is det, test_numeric_miner_requires_numeric_pattern() (+15 more)
...
Cohesion: 0.33
Nodes (9): measure(), cited_docs(), metrics(), evaluate(), run_retrieval_benchmark(), _doc(), _eval_item(), _unique() (+1 more)
...
### Community 133 - "TestRetrievalBenchmarkNdcg"
Cohesion: 0.29
...
- **Why does `Chunk` connect `SubjectSimJudge` to `.query`, `Chunk`, `.build`, `test_paraphrase_rescue.py`, `eval_harness.py`, `SpladeIndex`, `test_golden_v7_gate.py`, `test_selective_citations.py`, `RAGPipeline`, `test_attribution.py`, `context_headers.py`, `load_golden`, `load_circulars`, `HybridRetriever`, `ExtractiveStubGenerator`, `benchmark.py`, `test_rerank_jina_v3.py`, `validate_golden_v7`, `Lineage`, `test_pipeline.py`, `demote_superseded`, `build_spaces_pipeline`, `test_hyde.py`, `Qwen3MLXReranker`, `test_segment.py`, `resolve_chunk_spans`, `build_report`, `_confirms_claude`, `main`, `test_injection.py`, `main`, `test_benchmark.py`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `.query`, `Chunk`, `measure_context_precision`, `test_paraphrase_rescue.py`, `eval_harness.py`, `TestPerQueryRecall`, `load_golden`, `lineage.py`, `test_api.py`, `benchmark.py`, `settings.py`, `SubjectSimJudge`, `Lineage`, `build_spaces_pipeline`, `measure_supersession_precision`, `test_eval_harness_v7.py`, `Embedder`, `_unique`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `load_circulars()` connect `context_headers.py` to `Chunk`, `test_golden_v7_packet.py`, `SpladeIndex`, `RAGPipeline`, `backfill_escalations.py`, `test_segment.py`, `load_golden`, `derive_validity`, `SubjectSimJudge`?**
