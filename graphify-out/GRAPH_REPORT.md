...
## Corpus Check
- 234 files · ~217,607 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2943 nodes · 6166 edges · 194 communities (157 shown, 37 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 636 edges (avg confidence: 0.92)
...
## Communities (194 total, 37 thin omitted)
...
### Community 13 - "api.py"
Cohesion: 0.09
Nodes (24): BaseModel, main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), What actually makes a context window large: chunk size, or chunk count? Read-…, build_default_pipeline(), _citation_meta(), CitationMeta (+16 more)
...
Cohesion: 0.13
Nodes (34): main(), main(), main(), ADR-004 adoption: calibrate abstain_threshold for jina-reranker-v3-mlx's score…, main(), phase_judge(), Build the full pipeline with real models., real_pipeline() (+26 more)

### Community 22 - "Settings"
Cohesion: 0.18
Nodes (24): main(), _as_bool(), _get(), Path, Settings.load() plus the [spaces] table as settings.spaces.* Load order per…, Resolve a setting: env var > config dict > default., Coerce a config/env value to bool. Env vars arrive as strings; toml/default may…, Settings (+16 more)
...
### Community 24 - ".query"
Cohesion: 0.33
Nodes (3): Answer, _LineageAwareReranker, Reranker wrapper that re-applies lineage handling to its output. The paraphrase…
...
### Community 30 - "Chunk"
Cohesion: 0.09
Nodes (20): Generator, Judge, OllamaGenerator, Protocol, ADOPTED gate (eval_gate round 3): deterministic groundedness signal — max…, Max cosine(query, doc subject line) over contexts — the primary gate signal,…, Max cosine(query, section heading) over contexts — the second tier., Grounded generation via local Ollama (D6 canonical runtime option).… (+12 more)

### Community 31 - ".build"
Cohesion: 0.13
Nodes (19): expand_query(), Query-side lexical expansion for BM25 (intervention #2, glossary variant). SEBI…, Append statutory synonyms for lay tokens present in `query`. Deterministic and…, BM25 lexical index (bm25s)., SparseIndex, _chunk(), Query-side lexical expansion (intervention #2, glossary variant).…, test_all_five_sparse_failure_queries_expand() (+11 more)
...
Cohesion: 0.13
Nodes (26): answer_with_abstention(), faithfulness(), Check that every circular id the answer cites (in square brackets) was actually…, test_faithfulness_scoring(), _chunk(), Offline tests for the groundedness abstention gate (ADR-001 item 7)., rerank_top exactly at 0.85 overrides judge abstention (HYBRID_THRESHOLD=0.85)., rerank_top just below 0.85 does NOT override judge abstention. (+18 more)
...
Cohesion: 0.07
Nodes (5): app_module(), fixture, HF Spaces demo (root app.py): citations table + preview accordion logic. Fully…, app.py does `import spaces` (ZeroGPU) at module scope; stub it., _stub_spaces_package()
...
Cohesion: 0.08
Nodes (38): contexts_for(), annotate_corpus(), demote_superseded(), Lineage, Path, Down-weight reranked (chunk, score) pairs from superseded circulars and re-…, Map any cited circular that is superseded -> the circular(s) superseding it.…, Update each corpus record's supersession_status + superseded_by + supersedes… (+30 more)
...
Cohesion: 0.17
Nodes (7): Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., Segmentation: hierarchical chunking + metadata + stable citation IDs. Minimal,…, # NOTE: We intentionally DO NOT set carry here — flush() already, Step 12 — end-to-end RAG integration test with the REAL stack. bge-m3 (MPS) +…, _chunks(), Index persistence round-trip (offline)., test_index_save_load_roundtrip()
...
Cohesion: 0.15
Nodes (19): hierarchical_chunk(), Document -> section -> paragraph chunks with stable IDs. A "section" is…, test_numeric_miner_requires_numeric_pattern(), test_paraphrase_skips_preamble_and_short_chunks(), _ollama_up(), pipeline(), fixture, _body() (+11 more)
...
Cohesion: 0.18
Nodes (28): Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable…, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., RAGPipeline (+20 more)
...
### Community 101 - "remap_doc_ids.py"
Cohesion: 0.33
...
### Community 182 - "filter_targeted_rows"
Cohesion: 0.33
...
### Community 186 - "load_regulations"
Cohesion: 0.40
...
### Community 187 - "SetEncoderReranker"
Cohesion: 0.40
...
### Community 190 - "reg_display_name"
Cohesion: 0.50
...
- **Why does `Chunk` connect `Chunk` to `test_context_headers.py`, `load_circulars`, `test_paraphrase_rescue.py`, `generate.py`, `SpladeIndex`, `test_context_recall.py`, `test_selective_citations.py`, `api.py`, `test_attribution.py`, `scrape_sebi.py`, `HybridRetriever`, `.query`, `test_rerank_set_encoder.py`, `.build`, `answer_with_abstention`, `benchmark.py`, `resolve_chunk_spans`, `validate_golden_v7`, `test_lineage.py`, `TestRetrievalBenchmarkNdcg`, `SetEncoderReranker`, `test_rerank_jina_v3.py`, `test_spaces.py`, `segment.py`, `test_certainty.py`, `Qwen3MLXReranker`, `build_spaces_pipeline`, `hierarchical_chunk`, `.build_incremental`, `read_trec_run`, `RAGPipeline`, `test_hyde.py`, `test_injection.py`, `WarrantJudge`, `context_headers.py`, `paraphrase_rescue.py`, `main`, `.grounded`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `test_api.py`, `test_paraphrase_rescue.py`, `generate.py`, `api.py`, `HybridRetriever`, `.query`, `sebi_rag/eval_asof.py`, `run_all_metrics`, `Chunk`, `measure_context_precision`, `benchmark.py`, `_unique`, `test_lineage.py`, `measure_parsing_latency`, `TestRetrievalBenchmarkNdcg`, `hybrid_gate_sweep.py`, `TestPerQueryRecall`, `measure_supersession_precision`, `build_spaces_pipeline`, `sweep_citation_margin.py`, `test_eval_harness_v7.py`, `hierarchical_chunk`, `test_pipeline.py`, `measure.py`, `paraphrase_rescue.py`, `measure_mrr`, `measure_retrieval_recall`, `measure_temporal_accuracy`, `Embedder`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `HybridRetriever` to `run_judge`, `test_golden_v7_packet.py`, `Frame`, `generate.py`, `test_selective_citations.py`, `api.py`, `backfill_escalations.py`, `test_conformal.py`, `Settings`, `relabel_repooled.py`, `gemini_adjudicate.py`, `benchmark.py`, `agreement.py`, `local_adjudicate.py`, `main`, `main`, `hybrid_gate_sweep.py`, `adjudicate_draft.py`, `MeasureResult`, `seed_v7.py`, `test_eval_harness_v7.py`, `main`, `remap_doc_ids.py`, `main`?**
