...
## Corpus Check
- 254 files · ~246,599 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3521 nodes · 7212 edges · 222 communities (180 shown, 42 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 711 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `39d2bd1e`
...
- test_vectors_exposes_ndcg_for_floor_derivation
...
Cohesion: 0.08
Nodes (38): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production? The…, Preregistered cohort measurement for the CE paraphrase rescue. Spec:…, What does the 0.05 cross-encoder score floor actually catch?…, log(), Margin sweep for B' selective citations on the golden_v7 adjudicated set. One…, run(), Pool-width sweep (intervention #3): answer-level rescue rate vs reranker…, Benchmark MLX generators on the golden set: faithfulness, groundedness,… (+30 more)
...
Cohesion: 0.18
Nodes (17): Path, Resolution order: explicit SEBI_RAG_GOLDEN override, then the armed v7 gate,…, select_golden(), _gate_file(), Offline tests for the golden-v7 CI gate flip (spec 2026-07-23 sec 8). The gate…, floors_ok fails closed on a metric the report does not carry, so a floor name…, eval_json.py has no main() and boots real MPS models at import, so it cannot be…, A corrupt gate file must not arm the gate, and must not crash CI either - fall… (+9 more)
...
Cohesion: 0.05
Nodes (39): BaseModel, FastAPI, integration, _citation_meta(), CitationMeta, create_app(), get_chunk_text(), health() (+31 more)

### Community 12 - "HybridRetriever"
Cohesion: 0.12
Nodes (32): main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), Phase 1 (systematic-debugging) evidence gathering for the 2026-09-03…, main(), main(), main(), What actually makes a context window large: chunk size, or chunk count? Read-… (+24 more)
...
Cohesion: 0.17
Nodes (30): Build a lightweight pipeline for --smoke mode. Uses a stub retriever (no FAISS)…, smoke_pipeline(), smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network. Stable…, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., RAGPipeline (+22 more)
...
Cohesion: 0.13
Nodes (29): is_master(), main(), Is the eval set measuring retrieval, or measuring its own construction? Read-…, main(), R3 §3.1 — mine cross-reference (A cites B) candidate pairs. Spec:…, main(), _as_bool(), _get() (+21 more)
...
Cohesion: 0.14
Nodes (26): _unique(), main(), parse_args(), Namespace, Compare query-expansion arms (current prod / no-expand / HyDE) on a golden set.…, run_arm(), doc_ids_deduped(), fmt() (+18 more)
...
Cohesion: 0.13
Nodes (25): check_robots(), main(), Recover the 14 circular PDFs missed in the 2026-07-08 audit by resolving their…, Map each stem to (current pdf_url, detail_url) via listing sweeps., Log-only re-verification that our paths are still crawlable., resolve_stems(), stem_of(), main() (+17 more)
...
### Community 65 - "_chunk"
Cohesion: 0.15
Nodes (16): annotate_corpus(), Update each corpus record's supersession_status + superseded_by + supersedes…, _lin_chain(), P2 lineage / supersession resolution tests., test_annotate_corpus_adds_master_fields_and_consolidates_edges(), test_annotate_corpus_writes_new_metadata_fields(), test_build_lineage_edges_tiered(), test_build_lineage_inferred_master_topic_edge() (+8 more)
...
### Community 77 - "api.py"
Cohesion: 0.16
Nodes (9): main(), _compute_kwargs(), _embed_kwargs(), _compute_kwargs() plus model_path - BGEM3Embedder-only, never spread into…, Resolve device/fp16/batch for the torch embedder + reranker. Shared verbatim…, _embed_kwargs is _compute_kwargs + model_path, and must never mutate or widen…, test_compute_kwargs_cpu_disables_fp16(), test_compute_kwargs_mps_keeps_fp16() (+1 more)
...
Cohesion: 0.17
Nodes (10): Lineage, Path, Map any cited circular that is superseded -> the circular(s) superseding it.…, Connected component over supersedes/superseded_by (both tiers)., The circular in this family that governs on date as_of (ISO), or None when…, superseded_citations(), test_lineage_load_old_file_defaults_empty_edges(), test_lineage_save_load_roundtrip() (+2 more)
...
Cohesion: 0.14
Nodes (14): derive_floors(), metric -> per-query score vector, into gate-floor names -> floor value. Metrics…, Demotion or reranking can empty the context window while the fusion list is…, test_context_recall_floor_catches_a_regression_fusion_recall_misses(), The floor is a bootstrap lower bound minus a cushion, never the mean. Gating on…, A metric nobody scored must not be floored at 0 - a 0 floor is not a gate, it…, B' gates citation_precision alongside recall/recall_tradeoff/abstention. The…, recall_at_k is ceiling-limited (baseline 0.956 leaves ~9 failing queries of… (+6 more)
...
Cohesion: 0.40
Nodes (4): main(), parse_args(), Namespace, Sweep RRF k_const values on a golden set. No index rebuild needed. Turn 1 of…
...
### Community 131 - "_doc"
Cohesion: 0.12
Nodes (21): aggregate(), eligible(), main(), measure(), Preregistered cohort measurement for supersession confidence tiering. Spec:…, Answerable, non-as_of, with gold citations: the rows citation metrics exist for., cited_docs(), metrics() (+13 more)
...
### Community 148 - ".query"
Cohesion: 0.32
Nodes (11): Answer, answer_with_abstention(), _abstain(), _chunk(), Offline tests for the ADR-002 certainty architecture: abstention reasons,…, test_advisory_draft_on_gate_failure_only_when_requested(), test_certainty_capped_medium_without_gate(), test_certainty_high_when_subject_sim_strong_and_faithful() (+3 more)
...
Cohesion: 0.08
Nodes (16): Generator, Judge, OllamaGenerator, Protocol, Grounded generation via local Ollama (D6 canonical runtime option).…, _LineageAwareReranker, Reranker wrapper that re-applies lineage handling to its output. The paraphrase…, JinaMLXReranker (+8 more)
...
Cohesion: 0.12
Nodes (19): Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks., _is_table_row_candidate(), _is_table_row_filler(), _is_toc_row_candidate(), _merge_table_rows(), is_candidate(), Segmentation: hierarchical chunking + metadata + stable citation IDs. Minimal,…, Nesting depth of a numbered line ("2.1.3" -> 2), or None if it isn't one at all. (+11 more)
...
### Community 157 - "main"
Cohesion: 0.15
Nodes (6): _FakeRuntime, scripts/deploy_space.py: post-upload runtime read-back.  Covers the observabilit, Once the runtime's sha matches what was just pushed, a genuine     BUILD_ERROR f, The exact race this guards against: get_space_runtime called right     after upl, test_matching_sha_build_error_still_warns(), test_stale_runtime_sha_suppresses_warning()
...
### Community 162 - "test_settings_citation_scorer_enabled_true"
Cohesion: 0.17
Nodes (11): floors_ok(), Which golden set gates CI, and whether its adjudicated subset clears the…, True iff every floor's metric is present in `report_gate` and meets it. Missing…, derive_floors emits gate-report metric names, not the internal score_row names…, A floor naming a metric the report does not carry cannot be shown to hold, so…, test_floor_names_match_the_gate_report_keys(), test_floors_not_ok_when_one_metric_is_below(), test_floors_ok_is_inclusive_at_the_boundary() (+3 more)
...
### Community 168 - "test_settings_citation_scorer_enabled_false"
Cohesion: 0.31
...
Cohesion: 0.38
Nodes (6): _hardware_warning(), main(), Create/update the Gradio-SDK Hugging Face Space for the CPU demo and push app.py, Return a warning string if the just-deployed Space looks unhealthy,     else Non, Read back the Space's runtime after a deploy and print what CI was     previousl, _report_runtime()
...
### Community 201 - "run_retrieval_benchmark"
Cohesion: 0.33
Nodes (5): run_retrieval_benchmark(), _FixedOrderReranker, ADR-004: a reranker swap changes ORDER within top-10, which recall@10 (set…, Always returns the SAME doc order regardless of query — deterministic, so…, TestRetrievalBenchmarkNdcg
...
### Community 210 - "discover"
Cohesion: 0.60
Nodes (5): discover(), _parse_date(), parse_rows(), date, Extract (date, circular_url) pairs from a listing page, in page order.
...
### Community 216 - "_paragraphs"
Cohesion: 1.00
Nodes (3): _paragraphs(), add(), Split into units each <= max_chars. PDF-extracted text often lacks blank-line…
...
- **Why does `Chunk` connect `SetEncoderReranker` to `Chunk`, `.build`, `generate.py`, `test_injection.py`, `context_headers.py`, `test_paraphrase_rescue.py`, `test_context_recall.py`, `test_api.py`, `main`, `HybridRetriever`, `test_attribution.py`, `RAGPipeline`, `test_spaces.py`, `.query`, `test_benchmark.py`, `answer_with_abstention`, `segment.py`, `test_spaces_app.py`, `test_corpus.py`, `benchmark.py`, `WarrantJudge`, `test_selective_citations.py`, `hierarchical_chunk`, `test_rerank_set_encoder.py`, `test_rerank_jina_v3.py`, `test_splade_leg.py`, `_chunk`, `run_retrieval_benchmark`, `main`, `validate_golden_v7`, `test_lineage.py`, `corpus_spaces.py`, `retrieve.py`, `bench_rerankers.py`, `Qwen3MLXReranker`, `test_golden_v7_resolver.py`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `load_golden()` connect `HybridRetriever` to `_doc`, `run_judge`, `generate.py`, `main`, `Frame`, `main`, `seed_v7.py`, `test_finetune_eval_phase0.py`, `test_golden_v7_packet.py`, `test_conformal.py`, `segment.py`, `benchmark.py`, `test_finetune_holdout.py`, `gemini_adjudicate.py`, `backfill_escalations.py`, `local_adjudicate.py`, `agreement.py`, `test_selective_citations.py`, `relabel_repooled.py`, `MeasureResult`, `api.py`, `main`, `main`, `main`, `remap_doc_ids.py`, `hybrid_gate_sweep.py`, `adjudicate_draft.py`, `warrant_scorer_cohort.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `HybridRetriever` connect `.build` to `sweep_rrf_k.py`, `generate.py`, `run_judge`, `main`, `test_api.py`, `HybridRetriever`, `RAGPipeline`, `SetEncoderReranker`, `Settings`, `reranker_interaction_check.py`, `_strip_context_header`, `test_golden_v7_pool.py`, `test_splade_leg.py`, `main`, `api.py`, `main`, `corpus_spaces.py`, `retrieve.py`, `trace_failure.py`, `warrant_scorer_cohort.py`?**
