...
## Corpus Check
- 246 files · ~229,576 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3186 nodes · 6601 edges · 187 communities (147 shown, 40 thin omitted)
...
- test_section_score_exactly_at_threshold_passes
...
Cohesion: 0.10
Nodes (23): Ground truth: what do the 4 CE_MISMATCH rows actually DO in production? The…, Benchmark MLX generators on the golden set: faithfulness, groundedness,…, Retrieval-only benchmark with TREC runfile and reproducibility metadata. Use…, scripts/eval_asof.py, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the…, ADR-002 follow-up: compare the production subject-sim gate against the SECTION-…, False positive check — hybrid gate over abstain=True rows. Runs the pipeline…, Embedder protocol + a deterministic test embedder + the real bge-m3 embedder.… (+15 more)
...
Cohesion: 0.07
Nodes (42): _alias_keys(), derive_regulatory_basis(), _jaccard(), load_regulations(), name_tokens(), Path, Regulation identity + name resolution (spec 2026-07-23 §3.2, §3.6). Regulations…, Candidate alias lookup keys, most literal first. Both the raw normalised form… (+34 more)
...
Cohesion: 0.15
Nodes (27): is_master(), main(), Is the eval set measuring retrieval, or measuring its own construction? Read-…, main(), _as_bool(), _get(), Path, Settings.load() plus the [spaces] table as settings.spaces.* Load order per… (+19 more)
...
Cohesion: 0.13
Nodes (17): Score-floor false-abstention diagnostics (hybrid-gate-prereg 2026-08-13, §10b).…, expand_query(), Query-side lexical expansion for BM25 (intervention #2, glossary variant). SEBI…, Append statutory synonyms for lay tokens present in `query`. Deterministic and…, Stage-1 hybrid retrieval: dense (FAISS) + sparse (BM25) fused by RRF. Mandatory…, BM25 lexical index (bm25s)., SparseIndex, Query-side lexical expansion (intervention #2, glossary variant).… (+9 more)
...
### Community 44 - "acquire_missing_pdfs.py"
Cohesion: 0.18
...
Cohesion: 0.07
Nodes (45): contexts_for(), annotate_corpus(), demote_superseded(), detect_relations(), Lineage, Path, Down-weight reranked (chunk, score) pairs from superseded circulars and re-…, Map any cited circular that is superseded -> the circular(s) superseding it.… (+37 more)
...
### Community 61 - "scrape_regulations.py"
Cohesion: 0.26
...
### Community 67 - ".build"
Cohesion: 0.11
Nodes (15): main(), SPIKE/GATE (throwaway, not preregistered) — R5's own precondition from the…, main(), What actually makes a context window large: chunk size, or chunk count? Read-…, main(), P0 prep: price a larger MLX generator before committing to the R0 upgrade.…, rss_gb(), build_default_pipeline() (+7 more)
...
### Community 70 - "build_supersession_pairs"
Cohesion: 0.24
...
### Community 76 - "paraphrase_rescue.py"
Cohesion: 0.20
...
### Community 90 - "mine_hard_negatives"
Cohesion: 0.24
...
Cohesion: 0.22
Nodes (8): main(), R3 §3.1 — mine cross-reference (A cites B) candidate pairs. Spec:…, detect_relations_ex(), Like detect_relations, but returns dict records with evidence spans., _window(), A circular that names another circular BEFORE the supersede trigger word must…, test_detect_relations_ex_evidence_and_extractor(), test_detect_relations_ex_supersedes_when_ref_before_trigger()
...
### Community 101 - "answer_with_abstention"
Cohesion: 0.39
...
Cohesion: 0.06
Nodes (64): _is_signoff_boilerplate(), _leaks_metadata(), load_chunks_by_doc(), load_corpus_records(), load_minable_docs(), main(), mine_citation_context(), mine_hard_negatives() (+56 more)
...
### Community 104 - "write_dataset_cards"
Cohesion: 0.22
...
### Community 105 - "test_acquire_missing.py"
Cohesion: 0.29
...
### Community 107 - "bench_rerankers.py"
Cohesion: 0.11
Nodes (23): aggregate(), eligible(), main(), measure(), Preregistered cohort measurement for supersession confidence tiering. Spec:…, Answerable, non-as_of, with gold citations: the rows citation metrics exist for., auroc(), best_threshold() (+15 more)
...
### Community 118 - "validate_corpus.py"
Cohesion: 0.43
...
Cohesion: 0.10
Nodes (12): Preregistered cohort measurement for the CE paraphrase rescue. Spec:…, What does the 0.05 cross-encoder score floor actually catch?…, cited_docs(), metrics(), Capture-once margin sweep for B' selective citations. One pipeline pass over…, Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped…, Build the dense+sparse index once and persist it (run after corpus changes).…, CE_MISMATCH false-abstention investigation (score-floor-diagnostic 2026-08-18).… (+4 more)

### Community 194 - "HybridRetriever"
Cohesion: 0.14
Nodes (36): main(), main(), main(), ADR-004 adoption: calibrate abstain_threshold for jina-reranker-v3-mlx's score…, main(), main(), _load_items(), main() (+28 more)
...
- **Why does `Chunk` connect `Chunk` to `context_headers.py`, `test_paraphrase_rescue.py`, `generate.py`, `Qwen3MLXReranker`, `test_golden_v7_gate.py`, `test_selective_citations.py`, `test_attribution.py`, `sebi_rag/verify_master.py`, `SpladeIndex`, `test_rerank_set_encoder.py`, `api.py`, `test_expand.py`, `.grounded`, `benchmark.py`, `test_spaces_app.py`, `Embedder`, `test_lineage.py`, `test_hyde.py`, `test_rerank_jina_v3.py`, `SpacesSettings`, `hierarchical_chunk`, `HybridRetriever`, `.build`, `load_circulars`, `paraphrase_rescue.py`, `Lineage`, `test_benchmark.py`, `test_gate.py`, `RAGPipeline`, `test_injection.py`, `answer_with_abstention`, `WarrantJudge`, `main`, `validate_golden_v7`, `resolve_chunk_spans`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `RAGPipeline` to `SparseIndex`, `test_api.py`, `test_paraphrase_rescue.py`, `generate.py`, `Chunk`, `sebi_rag/eval_asof.py`, `api.py`, `benchmark.py`, `acquire_missing_pdfs.py`, `Embedder`, `test_lineage.py`, `test_hyde.py`, `hybrid_gate_sweep.py`, `scrape_regulations.py`, `HybridRetriever`, `.build`, `build_supersession_pairs`, `paraphrase_rescue.py`, `Lineage`, `test_eval_harness_v7.py`, `mine_hard_negatives`, `test_acquire_missing.py`, `bench_rerankers.py`, `test_incremental_index.py`, `is_degenerate`, `validate_corpus.py`, `test_integration_e2e.py`, `.encode`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `sebi_rag/__init__.py` connect `bench_rerankers.py` to `SpacesSettings`, `test_ui.py`?**
