# Graph Report - SEBI circular RAG  (2026-07-29)
...
Cohesion: 0.17
Nodes (26): Any, beir_corpus_rows(), beir_query_rows(), build_golden_v6(), dir_fingerprint(), enrich_golden_item(), export_beir(), git_commit() (+18 more)
...
Cohesion: 0.13
Nodes (35): BaseModel, main(), main(), main(), build_default_pipeline(), CitationMeta, QueryRequest, QueryResponse (+27 more)
...
Cohesion: 0.11
Nodes (22): Protocol, Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Build the dense+sparse index once and persist it (run after corpus changes)., Calibrate top_k and the abstention threshold against the citation-precision sign, Run eval/golden/golden_asof_v1.jsonl (selector + pipeline modes) against the per, Emit one JSON line of retrieval/citation/abstention metrics using the persisted, Load the real SEBI circular corpus (data/corpus/circulars.jsonl) into chunks. (+14 more)
...
Cohesion: 0.14
Nodes (21): load_circulars(), Path, hierarchical_chunk(), _paragraphs(), Split into units each <= max_chars.      PDF-extracted text often lacks blank-li, Document -> section -> paragraph chunks with stable IDs.      A "section" is det, _pipeline(), _chunks() (+13 more)
...
Cohesion: 0.11
Nodes (16): FastAPI, _citation_meta(), create_app(), FastAPI service tests (offline pipelines): endpoints, auth, rate limit, metadata, /ready should trigger pipeline build and return ready=true., _slow_pipeline(), _SlowGenerator, test_auth_required_when_key_set() (+8 more)
...
Cohesion: 0.50
Nodes (3): _lin_chain(), P4b: as-of golden evaluation runner tests (offline)., test_run_selector_cases_pass_and_fail()
...
Cohesion: 0.11
Nodes (19): mrr(), ndcg_at_k(), Minimal retrieval metrics (subset of docs/project_context.md section 7).  Recall, recall_at_k(), Reciprocal Rank Fusion. Rank-only — sidesteps score-scale mismatch., rrf_fuse(), _ollama_up(), Step 12 — end-to-end RAG integration test with the REAL stack.  bge-m3 (MPS) + b (+11 more)
...
### Community 70 - "Chunk"
Cohesion: 0.12
Nodes (29): smoke_pipeline(), HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, ExtractiveStubGenerator, Deterministic: returns the top context text. No model required., CircularMeta, _CannedGenerator, _distinct_pipeline() (+21 more)
...
Cohesion: 0.19
Nodes (12): AsofCaseResult, load_golden_asof(), Path, As-of-date golden evaluation runner (P4b).  Two case modes drawn from eval/golde, Aggregate case results with an exact confidence interval.      Pure function of, run_pipeline_cases(), run_selector_cases(), summarize() (+4 more)
...
### Community 101 - "Chunk"
Cohesion: 0.17
Nodes (5): BenchmarkIssue, _span_resolution_issues(), Max cosine(query, doc subject line) over contexts — the primary         gate sig, Max cosine(query, section heading) over contexts — the second tier., Chunk
...
- **Why does `Chunk` connect `Chunk` to `Benchmark Infrastructure`, `.grounded`, `Lineage`, `Embedder`, `Master Metadata`, `Chunk`, `test_gate.py`, `detect_relations_ex`, `annotate_corpus`, `bench_rerankers.py`, `bench_retrieval.py`, `.encode`, `build_index.py`, `SparseIndex`, `LexicalReranker`, `validate_golden_v7`, `Chunk`, `resolve_chunk_spans`, `_alias_keys`, `test_faithfulness.py`, `verify_master.py`, `test_injection.py`, `main`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `Lineage` to `Benchmark Infrastructure`, `Chunk`, `Chunk`, `resolve_chunk_spans`, `TestPerQueryRecall`, `Embedder`, `.encode`, `answer_with_abstention`, `TestReadTrecRun`, `test_eval_harness_v7.py`, `load_golden_asof`, `test_gate.py`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `Lineage` connect `Lineage` to `Chunk`, `resolve_chunk_spans`, `test_build_reg_edges.py`, `load_regulations`, `Embedder`, `answer_with_abstention`, `load_golden_asof`, `corpus_spaces.py`, `test_gate.py`?**
