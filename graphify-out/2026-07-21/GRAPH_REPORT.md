...
## Summary
- 1115 nodes · 2403 edges · 54 communities (46 shown, 8 thin omitted)
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 541 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0105c59b`
...
Cohesion: 0.43
Nodes (7): AsofCaseResult, As-of-date golden evaluation runner (P4b).  Two case modes drawn from eval/golde, run_pipeline_cases(), run_selector_cases(), summarize(), RAGPipeline, test_summarize_reports_accuracy_and_failures()
...
Cohesion: 0.10
Nodes (14): bootstrap_ci(), BootstrapCI, paired_delta(), PairedResult, Uncertainty quantification for benchmark runs.  The golden set is n=56 answerabl, True when the randomization test rejects at 1 - confidence AND the         paire, Percentile bootstrap interval for the mean of per-query scores., Compare run `b` against run `a` on their shared queries.      Returns mean_b - m (+6 more)
...
Cohesion: 0.17
Nodes (11): Benchmark MLX generators on the golden set: faithfulness, groundedness, abstenti, Retrieval-only benchmark with TREC runfile and reproducibility metadata.  Use --, Answer, Generation with a hard abstention gate (D5).  If the top reranked score is below, End-to-end wiring: segment -> hybrid retrieve -> rerank -> generate/abstain., Stage-2 reranking (mandatory, D4). Cross-encoder in production; a deterministic, Segmentation: hierarchical chunking + metadata + stable citation IDs.  Minimal,, P1 evaluation-harness test (offline).  Loads the real seed corpus (data/corpus/c (+3 more)
...
Cohesion: 0.05
Nodes (39): Embedder, ndarray, _tokens(), DenseIndex, _doc_checksum(), HybridRetriever, ndarray, Path (+31 more)
...
Cohesion: 0.06
Nodes (69): FastAPI, smoke_pipeline(), create_app(), load_circulars(), Path, HashEmbedder, Deterministic hashed bag-of-words embedding. No model, no network.      Stable a, mrr() (+61 more)
...
Cohesion: 0.18
Nodes (17): BaseModel, build_default_pipeline(), _citation_meta(), CitationMeta, QueryRequest, QueryResponse, FastAPI service over the SEBI Circular RAG pipeline.  Run (real stack; loads the, BGEM3Embedder (+9 more)
...
Cohesion: 0.11
Nodes (26): main(), build_lineage(), _currency(), detect_relations(), detect_relations_ex(), load_records(), Like detect_relations, but returns dict records with evidence spans., Return (relation, referenced_circular) for each distinct reference. (+18 more)

### Community 31 - "main"
Cohesion: 0.15
Nodes (11): annotate_corpus(), Lineage, Path, Update each corpus record's supersession_status + superseded_by + supersedes, Connected component over supersedes/superseded_by (both tiers)., The circular in this family that governs on date as_of (ISO), or         None wh, test_annotate_corpus_adds_master_fields_and_consolidates_edges(), test_annotate_corpus_writes_new_metadata_fields() (+3 more)
...
### Community 49 - ".encode"
Cohesion: 0.22
Nodes (5): Build eval/golden/golden_v4.jsonl for the larger corpus. Each query is mapped to, ADR-002 follow-up: compare the production subject-sim gate against the SECTION-A, mc_topic(), P2 — cross-document supersession resolution.  Classifies each circular's referen, Normalised topic of a 'Master Circular for/on <TOPIC>' title, else None.      Us

### Community 50 - "renumber.py"
Cohesion: 0.25
Nodes (7): contexts_for(), demote_superseded(), Down-weight reranked (chunk, score) pairs from superseded circulars and     re-s, Map any cited circular that is superseded -> the circular(s) superseding it., superseded_citations(), test_demote_superseded_puts_in_force_on_top(), test_superseded_citations_flagged_for_retrieval()

### Community 51 - "_s_header_token"
Cohesion: 0.38
...
- **Why does `Chunk` connect `Reranking` to `Benchmark Infrastructure`, `Data Processing`, `Index & Evaluation`, `.grounded`, `HybridGenerator`, `.grounded`, `Benchmark Scripts`, `Qwen3MLXReranker`, `bench_rerankers.py`, `bench_retrieval.py`, `Embedder`, `Master Metadata`, `renumber.py`, `answer_with_abstention`, `Chunk`, `Answer Generation`, `SubjectSimJudge`, `test_gate.py`?**
  _High betweenness centrality (0.173) - this node is a cross-community bridge._
- **Why does `normalize_circular_number()` connect `Data Processing` to `Benchmark Infrastructure`, `Dataset Export`, `test_integration_e2e.py`, `As-of Evaluation`, `Corpus Validation`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `RAGPipeline` connect `Index & Evaluation` to `Benchmark Infrastructure`, `HybridGenerator`, `Benchmark Scripts`, `Embedder`, `renumber.py`, `Reranking`, `Answer Generation`, `SubjectSimJudge`, `main`?**
